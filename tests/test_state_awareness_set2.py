...
def test_journey_task_conversation_are_distinct():
    r=runtime(); j=r.ensure_journey("T-1","Goal","C-1")
    assert j.journey_id!="T-1" and j.conversation_id=="C-1" and j.task_id=="T-1"

def test_history_uses_recorded_events():
    r=runtime(); r.ensure_journey("T-1","Goal","C-1")
    r.record_event("T-1","TASK_STARTED",actor="dharen",new_state="RUNNING")
    s=r.situation("T-1",SituationLevel.HISTORY)
    assert s.truth_class is TruthClass.RECORDED_FACT and s.history[0]["type"]=="TASK_STARTED"

def test_missing_checkpoint_does_not_fabricate_current():
    r=runtime(); r.ensure_journey("T-1","Goal","C-1")
    assert r.situation("T-1",SituationLevel.CURRENT).status=="NO_AUTHORITATIVE_RECORD"

def test_next_uses_recorded_remaining_steps():
    r=runtime(); r.ensure_journey("T-1","Goal","C-1")
    r.checkpoint("T-1","verify","verify",["prepare"],["contradiction review"],"veridat","verification","ACTIVE")
    s=r.situation("T-1",SituationLevel.NEXT)
    assert s.status=="RECORDED_NEXT_STEP" and s.next["description"]=="contradiction review"

def test_blocked_next_is_not_projected_as_success():
    r=runtime(); r.ensure_journey("T-1","Goal","C-1")
    r.checkpoint("T-1","verify","verify",[],["review"],"veridat","verification","BLOCKED","missing evidence")
    s=r.situation("T-1",SituationLevel.NEXT)
    assert s.truth_class is TruthClass.BLOCKED and s.next["type"]=="BLOCKED"

def test_multiple_active_tasks_are_ambiguous():
    r=runtime(); r.ensure_journey("T-1","G1","C1"); r.ensure_journey("T-2","G2","C2")
    assert r.resolve()==(None,None)

def test_training_and_heldout_are_disjoint_and_valid():
    root=Path(__file__).resolve().parents[2]
    train=[json.loads(x) for x in (root/"data/training/set2_state_awareness.jsonl").read_text().splitlines() if x.strip()]
    test=[json.loads(x) for x in (root/"data/testing/set2_state_awareness_heldout.jsonl").read_text().splitlines() if x.strip()]
