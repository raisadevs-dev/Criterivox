...
class DharenHomePage extends StatelessWidget {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Text('Dharen',
              style: TextStyle(
                  color: t.text, fontSize: 25, fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text('Analysis Home • active work and completed analytical results',
              style: TextStyle(color: t.mutedText, fontSize: 11)),
          const SizedBox(height: 18),
          _section(context, 'ACTIVE ANALYSES', _active,
              Icons.play_circle_outline_rounded),
          const SizedBox(height: 16),
          _section(context, 'COMPLETED ANALYSES', _completed,
  ...
  Widget _section(BuildContext context, String title,
      List<PresentationState> items, IconData icon) {
    final t = CriterivoxTheme.of(context);
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Icon(icon, color: t.primary, size: 19),
          ...
          if (items.isEmpty)
            Text('No analyses in this section yet.',
      ...
      child: Row(
        children: [
          Icon(
              ...
              color: completed ? t.success : t.primary,
              size: 19),
          const SizedBox(width: 10),
          Expanded(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.taskId ?? 'Unknown task',
                  style: TextStyle(
                      color: t.text,
                      fontSize: 10.5,
                      fontWeight: FontWeight.w700)),
              const SizedBox(height: 3),
              Text(item.task ?? 'Analysis task',
                  maxLines: 2,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(color: t.mutedText, fontSize: 10)),
              const SizedBox(height: 3),
              Text(item.taskState ?? 'UNKNOWN',
                  style: TextStyle(
                      color: t.mutedText,
                      fontSize: 8.5,
                      fontWeight: FontWeight.w700)),
