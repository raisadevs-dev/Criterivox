...
class _ConfirmationState extends State<_Confirmation>{final correction=TextEditingController();bool busy=false;@override void dispose(){correction.dispose();super.dispose();}
 Future<void> act(bool yes)async{setState(()=>busy=true);try{final w=await widget.client.confirm(widget.work['work_id'],yes,correction:yes?null:correction.text);if(!mounted)return;if(yes)Navigator.pushReplacement(context,MaterialPageRoute(builder:(_)=>_Review(client:widget.client,work:w)));else setState(()=>busy=false);}catch(e){if(mounted){setState(()=>busy=false);ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('$e')));}}}
...
class _ReviewState extends State<_Review>{late Map<String,dynamic>w;final challenge=TextEditingController();final modification=TextEditingController();bool busy=false;@override void initState(){super.initState();w=widget.work;}@override void dispose(){challenge.dispose();modification.dispose();super.dispose();}
 Future<void> refresh(Future<Map<String,dynamic>> f)async{setState(()=>busy=true);try{setState(()=>w=await f);}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('$e')));}finally{if(mounted)setState(()=>busy=false);}}
 @override Widget build(BuildContext c){final arts=List<Map<String,dynamic>>.from(w['artifacts']??[]);final opts=arts.isEmpty?<Map<String,dynamic>>[]:List<Map<String,dynamic>>.from(arts.last['options']??[]);return Scene(eyebrow:'HUMAN RESIDENCE · REVIEW',title:'Review Work',subtitle:'TAKE means you review the prepared work. It never means the system made your decision.',children:[
  ...
  for(final a in arts)SpatialPanel(title:'PREPARED STRATEGIES',child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:opts.map((o)=>ListTile(title:Text('\${o['title']}'),subtitle:Text('\${o['description']}'),trailing:FilledButton(onPressed:busy?null:()=>refresh(widget.client.decide(w['work_id'],o['option_id'],modification.text)),child:const Text('SELECT'))).toList())),
  SpatialPanel(title:'HUMAN CHALLENGE',child:Column(children:[TextField(controller:challenge,decoration:const InputDecoration(border:OutlineInputBorder(),hintText:'Challenge premise, evidence or assumptions.')),Wrap(spacing:8,children:[OutlinedButton(onPressed:busy?null:()=>refresh(widget.client.challenge(w['work_id'],'PREMISE',challenge.text)),child:const Text('CHALLENGE PREMISE')),OutlinedButton(onPressed:busy?null:()=>refresh(widget.client.challenge(w['work_id'],'EVIDENCE',challenge.text)),child:const Text('REQUEST EVIDENCE'))])]),
  _ResearchPanel(client:widget.client,work:w,onUpdate:(x)=>setState(()=>w=x)),
  if(w['status']=='DECISION_READY')SpatialPanel(title:'ACTION SAFETY GATE',child:FilledButton(onPressed:busy?null:()=>refresh(widget.client.authorize(w['work_id'])),child:const Text('AUTHORIZE ACTION'))),
 ]);}}
