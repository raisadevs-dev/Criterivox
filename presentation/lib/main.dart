...
class _CriterivoxScreenState extends State<CriterivoxScreen> {
  ...
  void _createWorkspaceTask() {
    ...
    _runtime.requestApplication(intent: 'analyze', task: task,
      data: {'dataset': _dataController.text.trim(), 'records': 3},
      context: {'description': _contextController.text.trim(), 'origin': 'Analysis Workspace'}, source: 'workspace');
  }
  ...
  void _sendFollowup() {
    ...
    _chatController.clear();
    _runtime.sendChat(taskId: state!.taskId, message: text);
  }

  @override
  Widget build(BuildContext context) => Scaffold(
    body: Stack(children: [const Positioned.fill(child: _Backdrop()), SafeArea(child: Column(children: [
      const _Header(),
  ...
  Widget _surfaceBody(bool compact) {
    if (_surface == 'bloom') return Column(children: [
      _Title('Capability Gateway', 'Choose a capability. Analyze is the active S4 experience.'),
      _BloomCard(), if (_chooser) _EntryChooser(),
    ]);
  ...
  Widget _EntryChooser() => Padding(padding: const EdgeInsets.only(top: 14), child: _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 5), const Text('Two entry points. One analysis task. One Dharen.', style: TextStyle(color: Color(0xFFAAB0C8))),
    const SizedBox(height: 18), Row(children: [
      Expanded(child: _ActionCard(icon: Icons.grid_view_rounded, title: 'Open Analysis Workspace', text: 'Work visually with task, context, observations, findings, and Dharen.', onTap: _openWorkspace)),
      const SizedBox(width: 12), Expanded(child: _ActionCard(icon: Icons.chat_bubble_outline_rounded, title: 'Chat with Dharen', text: 'Give Dharen the analysis request directly through his inbox.', onTap: _openChat)),
    ]),
  ])));

  Widget _WorkspaceHeader() => Row(children: [
    IconButton(onPressed: () => setState(() => _surface = 'bloom'), icon: const Icon(Icons.arrow_back_rounded)),
    const SizedBox(width: 4), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Analysis Workspace', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700)),
      Text(_state?.taskId == null ? 'Dharen’s Home • New analysis' : 'Dharen’s Home • Task ${_state!.taskId}', style: const TextStyle(color: Color(0xFF9BA2BF), fontSize: 12)),
    ])),
    _StatusPill(_state?.taskState ?? 'NO TASK'), const SizedBox(width: 10),
    OutlinedButton.icon(onPressed: _openChat, icon: const Icon(Icons.chat_bubble_outline_rounded, size: 16), label: const Text('Chat with Dharen')),
  ]);

  Widget _PrimaryPanel() => _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [const Icon(Icons.analytics_outlined, color: Color(0xFF8E7BFF)), const SizedBox(width: 9), const Text('Analysis Task', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)), const Spacer(), if (_state?.taskId != null) Text(_state!.taskId!, style: const TextStyle(color: Color(0xFF888FAE), fontSize: 11))]),
  ...
  Widget _TaskComposer() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('Start an analysis', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600)), const SizedBox(height: 5),
    const Text('The information below becomes the shared Analysis Task used by Workspace and Dharen Inbox.', style: TextStyle(color: Color(0xFFA5ABC2), fontSize: 12)), const SizedBox(height: 18),
    _Field(label: 'Task', controller: _taskController, maxLines: 2), const SizedBox(height: 12),
    _Field(label: 'Data', controller: _dataController), const SizedBox(height: 12), _Field(label: 'Context', controller: _contextController), const SizedBox(height: 16),
  ...
  Widget _LiveTask() { final s = _state!; return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [_StatusPill(s.taskState ?? 'UNKNOWN'), const SizedBox(width: 10), Expanded(child: Text(s.task ?? '', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600))),]),
    const SizedBox(height: 16), _Lifecycle(state: s.taskState ?? 'CREATED'), const SizedBox(height: 16),
    Row(children: [Expanded(child: _Metric(label: 'Data fields', value: '${s.taskDataFields ?? 0}')), Expanded(child: _Metric(label: 'Context fields', value: '${s.taskContextFields ?? 0}')), Expanded(child: _Metric(label: 'Observations', value: '${s.observations.length}')), Expanded(child: _Metric(label: 'Findings', value: '${s.findings.length}'))]),
  ]); }

  Widget _WorkspaceData() => _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('Data & Context', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const SizedBox(height: 12),
    _InfoLine(Icons.storage_outlined, 'Data', _state?.taskDataFields == null ? 'Awaiting task' : '${_state!.taskDataFields} top-level fields recorded'),
    _InfoLine(Icons.account_tree_outlined, 'Context', _state?.taskContextFields == null ? 'Awaiting task' : '${_state!.taskContextFields} contextual fields recorded'),
    _InfoLine(Icons.link_rounded, 'Source', _state?.taskSource ?? 'Not started'),
  ]));

  Widget _DharenInbox() => _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [const Icon(Icons.forum_outlined, color: Color(0xFF9B86FF)), const SizedBox(width: 8), const Text('Dharen Inbox', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const Spacer(), _StatusPill(_state?.taskState ?? 'IDLE')]),
    const SizedBox(height: 12),
    if (_state == null || _state!.taskId == null) const _EmptyInbox() else ...[
      _ChatBubble(fromUser: false, text: _state!.message ?? 'Dharen is ready.'),
      const SizedBox(height: 9),
      if (_state!.taskState == 'COMPLETED') _ChatBubble(fromUser: false, text: 'The analysis is complete. The result is available in this workspace.'),
    ],
    const SizedBox(height: 12),
    Row(children: [Expanded(child: TextField(controller: _chatController, minLines: 1, maxLines: 3, onSubmitted: (_) => _state?.taskId == null ? _createChatTask() : _sendFollowup(), decoration: InputDecoration(hintText: _state?.taskId == null ? 'Start a task with Dharen…' : 'Ask Dharen about this task…', filled: true, fillColor: const Color(0x6610142E), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)))), const SizedBox(width: 7), IconButton.filled(onPressed: _state?.taskId == null ? _createChatTask : _sendFollowup, icon: const Icon(Icons.arrow_upward_rounded))]),
  ]));

  Widget _ActivityAndResults() => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Expanded(child: _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Recent Observations', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const SizedBox(height: 10), if (_state == null || _state!.observations.isEmpty) const Text('Observations will appear here when the task produces them.', style: TextStyle(color: Color(0xFF8E95AE), fontSize: 12)) else for (final o in _state!.observations) _Observation(text: '${o['text'] ?? ''}', tag: '${o['significance'] ?? 'observed'}')]))),
    const SizedBox(width: 14),
    Expanded(child: _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Findings & Evidence', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const SizedBox(height: 10), if (_state == null || _state!.findings.isEmpty) const Text('Findings will appear here after analysis.', style: TextStyle(color: Color(0xFF8E95AE), fontSize: 12)) else for (final f in _state!.findings) _Finding(text: '${f['statement'] ?? ''}'), if (_state?.evidence.isNotEmpty == true) ...[const Divider(height: 24), const Text('Evidence', style: TextStyle(fontWeight: FontWeight.w600)), for (final e in _state!.evidence) _Observation(text: '${e['label'] ?? ''}: ${e['detail'] ?? ''}', tag: '${e['source'] ?? 'source'}')]]))),
    const SizedBox(width: 14),
...
class _Lifecycle extends StatelessWidget { final String state; const _Lifecycle({required this.state}); @override Widget build(BuildContext c) { const states = ['CREATED','RECEIVED','VALIDATING','PROCESSING','ANALYZING','RESULT_READY','COMPLETED']; final index = states.indexOf(state); return SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [for (var i=0;i<states.length;i++) ...[Container(width: 10, height: 10, decoration: BoxDecoration(shape: BoxShape.circle, color: i <= index ? const Color(0xFF8C75FF) : const Color(0xFF303550))), if (i < states.length-1) Container(width: 35, height: 1, color: i < index ? const Color(0xFF6C5AB9) : const Color(0xFF252A43)), if (i == states.length-1) const SizedBox(width: 5)],])); } }
