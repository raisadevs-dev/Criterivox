...
class PresentationState {
  ...
  /// an explicit replacement is supplied.
  PresentationState copyWith({
    ...
    String? foundationPreviewQuestion,
    double? foundationMatchRatio,
    bool? foundationAutoFill,
    List<String>? foundationIntentGuesses,
    String? foundationRecipient,
    int? foundationLogCount,
    List<String>? foundationLogEntries,
    List<String>? foundationConflictFields,
    List<String>? foundationConditionalProvenance,
    String? contextId,
    List<String>? contextDimensions,
    List<String>? contextMissingDimensions,
    int? contextNormalizationCount,
    String? contextBaselineId,
    String? contextBaselineStatus,
    String? contextInterpretationId,
    List<String>? contextUncertainty,
    ...
    return PresentationState(
      ...
      task: task ?? this.task,
      taskDataFields: taskDataFields ?? this.taskDataFields,
      taskContextFields: taskContextFields ?? this.taskContextFields,
      ...
      foundationPreviewQuestion:
          foundationPreviewQuestion ?? this.foundationPreviewQuestion,
      foundationMatchRatio:
          foundationMatchRatio ?? this.foundationMatchRatio,
      foundationAutoFill: foundationAutoFill ?? this.foundationAutoFill,
      foundationIntentGuesses:
          foundationIntentGuesses ?? this.foundationIntentGuesses,
      foundationRecipient:
          foundationRecipient ?? this.foundationRecipient,
      foundationLogCount:
          foundationLogCount ?? this.foundationLogCount,
      foundationLogEntries:
          foundationLogEntries ?? this.foundationLogEntries,
      foundationConflictFields:
          foundationConflictFields ?? this.foundationConflictFields,
      foundationConditionalProvenance:
          foundationConditionalProvenance ??
              this.foundationConditionalProvenance,
      ...
      contextNormalizationCount:
          contextNormalizationCount ?? this.contextNormalizationCount,
      contextBaselineId:
          contextBaselineId ?? this.contextBaselineId,
      ...
      contextUncertainty:
          contextUncertainty ?? this.contextUncertainty,
      contextLimitations:
          contextLimitations ?? this.contextLimitations,
      ...
      contextStateVersion:
          contextStateVersion ?? this.contextStateVersion,
      contextCheckpointId:
          contextCheckpointId ?? this.contextCheckpointId,
      contextSandboxCount:
          contextSandboxCount ?? this.contextSandboxCount,
      contextTierBudget:
          contextTierBudget ?? this.contextTierBudget,
      contextAuthoritativeInput:
          contextAuthoritativeInput ?? this.contextAuthoritativeInput,
      provenanceGraph:
          provenanceGraph ?? this.provenanceGraph,
      contextDiff:
          contextDiff ?? this.contextDiff,
      evidenceCompleteness:
          evidenceCompleteness ?? this.evidenceCompleteness,
      evidenceDebtLevel:
          evidenceDebtLevel ?? this.evidenceDebtLevel,
      evidenceTags:
          evidenceTags ?? this.evidenceTags,
      memoryStatus:
          memoryStatus ?? this.memoryStatus,
      memoryRecheckAt:
          memoryRecheckAt ?? this.memoryRecheckAt,
      memoryRecheckReason:
          memoryRecheckReason ?? this.memoryRecheckReason,
      observabilityEvents:
          observabilityEvents ?? this.observabilityEvents,
      failureId:
      ...
      lineageSnapshot:
          lineageSnapshot ?? this.lineageSnapshot,
      coordinationId:
          coordinationId ?? this.coordinationId,
      coordinationMembers:
          coordinationMembers ?? this.coordinationMembers,
      deliveryId:
  ...
  factory PresentationState.fromJson(String raw) {
    ...
    if (stateValue is! String ||
      ...
      throw const FormatException(
        'Runtime message has an unsupported state.',
      );
    }
    ...
    if (message != null && message is! String) {
      throw const FormatException(
        'Runtime message has invalid message.',
      );
    }

    if (event != null && event is! String) {
      throw const FormatException(
        'Runtime message has invalid event.',
      );
    }
    ...
    final contextTierBudget = map(
      decoded['context_tier_budget'],
    );
    ...
    return PresentationState(
      ...
      foundationSourceCount:
          integer(decoded['foundation_source_count']),
      foundationCandidateCount:
      ...
      foundationMatchRatio:
          decimal(decoded['foundation_match_ratio']),
      foundationAutoFill:
          optionalBool(decoded['foundation_auto_fill']),
      foundationIntentGuesses:
          strings(decoded['foundation_intent_guesses']),
      foundationRecipient:
          optionalString(decoded['foundation_recipient']),
      foundationLogCount:
          integer(decoded['foundation_log_count']),
      foundationLogEntries:
          strings(decoded['foundation_log_entries']),
      foundationConflictFields:
      ...
      contextDimensions:
          strings(decoded['context_dimensions']),
      contextMissingDimensions:
          strings(decoded['context_missing_dimensions']),
      contextNormalizationCount:
          integer(decoded['context_normalization_count']),
      contextBaselineId:
          optionalString(decoded['context_baseline_id']),
      contextBaselineStatus:
          optionalString(decoded['context_baseline_status']),
      contextInterpretationId:
          optionalString(decoded['context_interpretation_id']),
      contextUncertainty:
          strings(decoded['context_uncertainty']),
      contextLimitations:
          strings(decoded['context_limitations']),
      contextCompressionRatio:
          decimal(decoded['context_compression_ratio']),
      contextOriginalItemCount:
          integer(decoded['context_original_item_count']),
      contextRetainedItemCount:
          integer(decoded['context_retained_item_count']),
      contextViolationCount:
          integer(decoded['context_violation_count']),
      contextStateVersion:
          integer(decoded['context_state_version']),
      contextCheckpointId:
          optionalString(decoded['context_checkpoint_id']),
      contextSandboxCount:
          integer(decoded['context_sandbox_count']),
      contextTierBudget:
          contextTierBudget,
      contextAuthoritativeInput:
      ...
      provenanceGraph:
          map(decoded['provenance_graph']),
      contextDiff:
          map(decoded['context_diff']),

      evidenceCompleteness:
          integer(decoded['evidence_completeness']),
      evidenceDebtLevel:
          optionalString(decoded['evidence_debt_level']),
      evidenceTags:
          strings(decoded['evidence_tags']),

      memoryStatus:
          optionalString(decoded['memory_status']),
      memoryRecheckAt:
          optionalString(decoded['memory_recheck_at']),
      memoryRecheckReason:
          optionalString(decoded['memory_recheck_reason']),
      observabilityEvents:
          maps(decoded['observability_events']),

      failureId:
          optionalString(decoded['failure_id']),
      failureType:
          optionalString(decoded['failure_type']),
      executionEngine:
          optionalString(decoded['execution_engine']),
      executionTier:
          optionalString(decoded['execution_tier']),
      fallbackUsed:
          optionalBool(decoded['fallback_used']),
      doorAddress:
          optionalString(decoded['door_address']),

      lineageSnapshot:
          map(decoded['lineage_snapshot']),

      coordinationId:
          optionalString(decoded['coordination_id']),
      coordinationMembers:
          strings(decoded['coordination_members']),

      deliveryId:
          optionalString(decoded['delivery_id']),
      deliveryRecipient:
          optionalString(decoded['delivery_recipient']),
      deliveryStatus:
          optionalString(decoded['delivery_status']),

      observations:
          maps(decoded['observations']),
      findings:
          maps(decoded['findings']),
      evidence:
          maps(decoded['evidence']),
      activity:
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'foundation_conditional_provenance':
          foundationConditionalProvenance,
