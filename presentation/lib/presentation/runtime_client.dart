...
class CharacterRuntimeClient {
  ...
  Future<PresentationState?> restoreState()async{return null;}
  ...
  void _handleMessage(dynamic message){try{final state=PresentationState.fromJson(message.toString());_states.add(state);_persistStateAndFoundation(state);}on FormatException catch(error){_errors.add('Rejected runtime state: ${error.message}');}catch(_){_errors.add('Rejected runtime state.');}}
  Future<void> _persistStateAndFoundation(PresentationState state)async{final map=state.toJson();final foundationId=state.foundationId;if(foundationId!=null){final existing=await _foundations.get(foundationId);if(existing!=null){final revision=((existing['_runtime_revision'] as num?)?.toInt()??0)+1;existing['_runtime_revision']=revision;existing['_presentation_state']=map;await _persistFoundation(existing,revision:revision);}}}
