...
class CharacterRuntimeClient {
  ...
  Future<void> connect() async{if(_disposed||_connecting||_channel!=null)return;_connecting=true;_retryTimer?.cancel();try{await _hydrateResidency();final channel=WebSocketChannel.connect(endpoint);_channel=channel;await channel.ready;if(_disposed){await channel.sink.close();return;} _flushPending(channel);channel.stream.listen(_handleMessage,onError:(_)=>_connectionLost(),onDone:_connectionLost,cancelOnError:false);}catch(_){_channel=null;_scheduleReconnect();}finally{_connecting=false;}}
  Future<void> _hydrateResidency() async { await _residency.load(); }
  ...
  Future<void> _persistRequest(Map<String,dynamic> payload) async { await _residency.save({'schema_version':1,'updated_at':DateTime.now().toUtc().toIso8601String(),'last_request':payload}); }
  Future<void> _persistState(PresentationState state) async { final previous=await _residency.load() ?? <String,dynamic>{}; previous['schema_version']=1; previous['updated_at']=DateTime.now().toUtc().toIso8601String(); previous['presentation_state']=state.toJson(); if(state.foundationId!=null) previous['foundation_id']=state.foundationId; await _residency.save(previous); }
