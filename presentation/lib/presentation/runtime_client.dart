...
class CharacterRuntimeClient {
  ...
  Future<void> connect() async{if(_disposed||_connecting||_channel!=null)return;_connecting=true;_retryTimer?.cancel();try{final channel=WebSocketChannel.connect(endpoint);_channel=channel;await channel.ready;if(_disposed){await channel.sink.close();return;}_flushPending(channel);await _recoverBrowserFoundations(channel);channel.stream.listen(_handleMessage,onError:(_)=>_connectionLost(),onDone:_connectionLost,cancelOnError:false);}catch(error){_channel=null;_errors.add('Runtime connection failed: $error');_scheduleReconnect();}finally{_connecting=false;}}
