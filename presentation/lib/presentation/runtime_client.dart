  ...
  Future<void> _recoverBrowserFoundations(WebSocketChannel channel) async{
    ...
    try{
      ...
      for(final envelope in envelopes){
        ...
        final id='${envelope['foundation_id']??(foundation is Map?foundation['foundation_id']:'')}';
        final revision=int.tryParse('${envelope['revision']??1}')??1;
        if(foundation is! Map||id.isEmpty)continue;
        channel.sink.add(jsonEncode({'type':'foundation_sync','schema_version':1,'foundation_id':id,'revision':revision,'foundation':Map<String,dynamic>.from(foundation)}));
      }
    }finally{_foundationRecoveryInFlight=false;}
  }
