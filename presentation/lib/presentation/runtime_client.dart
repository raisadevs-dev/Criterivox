...
class CharacterRuntimeClient {
  ...
  void _scheduleReconnect() {
    if (_disposed || _retryTimer?.isActive == true) {
      return;
    }
  ...
  Future<void> _recoverBrowserFoundations(
    WebSocketChannel channel,
  ) async {
    if (_foundationRecoveryInFlight ||
    ...
    try {
      ...
      for (final envelope in envelopes) {
        ...
        final id =
            '${envelope['foundation_id'] ??
            (foundation is Map ? foundation['foundation_id'] : '')}';
        final revision =
            int.tryParse('${envelope['revision'] ?? 1}') ?? 1;

        if (foundation is! Map || id.isEmpty) {
          continue;
        }
  ...
  Future<Map<String, dynamic>?> createSandbox({
    ...
    Map<String, dynamic> variables = const {},
      ...
      _postContext(
        '/runtime/context/sandbox/create',
        {
          'foundation_id': foundationId,
          'variables': variables,
        },
  ...
  Future<Map<String, dynamic>?> inspectSandbox(
    String sandboxId,
  ) async {
    try {
  ...
  Future<Map<String, dynamic>?> promoteSandbox(
    String sandboxId,
  ) =>
      _postContext(
        '/runtime/context/sandbox/$sandboxId/promote',
        {},
      );

  Future<Map<String, dynamic>?> discardSandbox(
    String sandboxId,
  ) =>
      _postContext(
        '/runtime/context/sandbox/$sandboxId/discard',
        {},
      );
  ...
  void _send(Map<String, dynamic> payload) {
    ...
    if (channel == null) {
      if (_pending.length < _maxPending) {
        ...
        _errors.add(
          'Runtime queue is full; the oldest unsent action was preserved '
          'and this action was rejected.',
        );
      }
  ...
  void _handleMessage(dynamic message) {
    try {
      ...
      if (decoded is Map &&
          decoded['type'] == 'foundation_sync_ack') {
      ...
      if (decoded is Map &&
          decoded['message_type'] == 'context_handoff') {
