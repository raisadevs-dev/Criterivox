...
class CharacterChatPage extends StatefulWidget {
  ...
  final void Function(
          String message, String agent, List<Map<String, dynamic>> references)
      onSend;
...
class _CharacterChatPageState extends State<CharacterChatPage> {
  ...
  void didUpdateWidget(covariant CharacterChatPage old) {
    ...
    if (widget.state != old.state) _recordRuntimeMessage(widget.state);
  }

  void _recordRuntimeMessage(PresentationState? s) {
    if (s == null || s.event != 'CHARACTER_CHAT_RESPONSE') return;
  ...
  List<Map<String, dynamic>> get activeReferences =>
      references[widget.selectedAgent] ?? [];
  void _send() {
    final text = input.text.trim();
    if (text.isEmpty || widget.busy) return;
    setState(() {
      activeMessages.add(_ChatMessage(sender: 'user', text: text));
      input.clear();
    });
    widget.onSend(text, widget.selectedAgent, List.from(activeReferences));
  }

  void _sendChoice(String t) {
    if (widget.busy) return;
    setState(() => activeMessages.add(_ChatMessage(sender: 'user', text: t)));
    widget.onSend(t, widget.selectedAgent, List.from(activeReferences));
  }
  ...
  Future<void> _attach() async {
    ...
    if (f.bytes == null) {
      _error('The selected file could not be read in the browser.');
      return;
    }
    if (f.bytes!.length > 4 * 1024 * 1024) {
      _error('Reference files are limited to 4 MB each.');
      return;
    }
    setState(() => activeReferences.add({
          'name': f.name,
          'kind': 'reference',
          'size_bytes': f.bytes!.length,
          'content_base64': base64Encode(f.bytes!)
        }));
  }
              ...
              if (!narrow)
                SizedBox(
                    ...
                    child: _CharacterPicker(
                        selected: widget.selectedAgent,
                        onSelect: widget.onSelectAgent)),
              Expanded(
                  child: _Conversation(
                      state: widget.state,
                      busy: widget.busy,
    ...
    return Container(
        ...
        child: Column(children: [
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (c, i) {
                    ...
                    return ListTile(
                        ...
                        leading: SessionCharacterAnimationView(
                            characterId: m.id,
                            state: selected == m.id ? 'COMMUNICATE' : 'IDLE',
                            width: 48,
                            height: 56),
                        title: Text(m.name,
                            style: TextStyle(
          ...
          Padding(
              ...
              child: Text(
                  'Each character has an independent conversation history. Task, context, evidence and runtime state remain shared.',
                  style: TextStyle(
                      color: t.mutedText, fontSize: 10, height: 1.45)))
        ]));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Container(
          ...
          child: Row(children: [
            SessionCharacterAnimationView(
                characterId: target, state: rs, width: 60, height: 68),
      ...
      Expanded(
        ...
        if (state?.agentId.toLowerCase() == target &&
            state?.event == 'HANDOFF_PROPOSED')
          _HandoffChoices(
              busy: busy,
              onHandoff: () => onChoice('Hand over this task to Dharen.'),
              onContinue: () => onChoice('I will continue the task here.')),
        if (state?.agentId.toLowerCase() == target && state?.taskId != null)
          _TaskCard(state: state!, onOpen: onOpenTask)
      ])),
      _Composer(
          input: input,
...
class _Welcome extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    ...
    return Container(
        ...
        child: Text(
            '${member.name} workspace\n\nThis conversation belongs only to ${member.name}. Shared task and context state remains outside the conversation buffer.',
  ...
  Widget build(BuildContext c) {
    ...
    return Align(
        ...
        child: Container(
            ...
            decoration: BoxDecoration(
                color: u ? t.primary.withValues(alpha: .14) : t.surfaceStrong,
                borderRadius: BorderRadius.circular(16),
                border: Border.all(
                    color: u ? t.primary.withValues(alpha: .35) : t.border)),
            child:
              ...
              if (!u)
                Text(displayName,
                    style: TextStyle(
                        ...
                        fontWeight: FontWeight.w700)),
              Text(message.text,
                  style: TextStyle(color: t.text, fontSize: 12.5, height: 1.45))
            ])));
  }
}
        ...
        child: Row(children: [
          ...
          Expanded(
              child: TextField(
                  ...
                  decoration: InputDecoration(hintText: 'Message $name…'))),
...
class _HandoffChoices extends StatelessWidget {
  ...
  const _HandoffChoices(
      {required this.busy, required this.onHandoff, required this.onContinue});
  @override
  Widget build(BuildContext c) => Wrap(spacing: 8, children: [
        OutlinedButton(
            onPressed: busy ? null : onHandoff,
            child: const Text('Hand over to Dharen')),
        FilledButton(
...
class _ContextPanel extends StatelessWidget {
  ...
  final VoidCallback onOpenTask;
  const _ContextPanel(
      {required this.state, required this.target, required this.onOpenTask});
  @override
  Widget build(BuildContext c) {
    final t = CriterivoxTheme.of(c);
    final m = _CharacterChatPageState.members.firstWhere((x) => x.id == target);
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SessionCharacterAnimationView(
              characterId: target,
              state: state?.agentId.toLowerCase() == target
                  ? (state?.characterState ?? 'IDLE')
                  : 'IDLE',
              width: 120,
              height: 140),
          ...
          Text(m.role, style: TextStyle(color: t.mutedText, fontSize: 10)),
          const SizedBox(height: 20),
          Text('Task: ${state?.taskId ?? 'None'}'),
          Text('State: ${state?.taskState ?? 'IDLE'}'),
          Text('Context: ${state?.contextId ?? 'Not built'}'),
          Text('Evidence: ${state?.evidence.length ?? 0} items'),
          const Spacer(),
          if (state?.taskId != null)
            FilledButton(
                onPressed: onOpenTask, child: const Text('Open current task'))
        ]));
  }
}
