...
class _CharacterInfo {
  ...
  final String name;
  final String role;
  const _CharacterInfo(this.id,this.name,this.role);
}
...
class _CharacterChatPageState extends State<CharacterChatPage> {
  ...
  void initState() {
    ...
    for (final member in members) {
      conversations[member.id] = <_ChatMessage>[];
      references[member.id] = <Map<String,dynamic>>[];
    }
  ...
  void _recordRuntimeMessage(PresentationState? state) {
    ...
    if (state == null || message == null || message.isEmpty) return;
...
class _CharacterPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration:BoxDecoration(border:Border(right:BorderSide(color:theme.border))),
...
class _AgentTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      onTap:onTap,
      selected:selected,
      selectedTileColor:theme.primary.withValues(alpha:.12),
      leading:CharacterRuntimeView(characterId:member.id,state:selected?'COMMUNICATE':'IDLE',width:48,height:56),
      title:Text(member.name,style:TextStyle(color:theme.text,fontSize:13,fontWeight:FontWeight.w600)),
      subtitle:Text(member.role,style:TextStyle(color:theme.mutedText,fontSize:9)),
    );
  }
}
...
class _Conversation extends StatelessWidget {
  ...
  final VoidCallback onSend,onAttach,onOpenTask;
  final ValueChanged<String> onSelectAgent,onChoice;
  ...
  Widget build(BuildContext context) {
    final theme = CriterivoxTheme.of(context);
    final member = _CharacterChatPageState.members.firstWhere((item)=>item.id==target);
    final runtimeState = state?.agentId.toLowerCase()==target ? (state?.characterState??'IDLE') : 'IDLE';
    final promptList = _CharacterChatPageState.prompts[target] ?? const <String>[];
    return Column(children:[
      Container(height:86,padding:const EdgeInsets.symmetric(horizontal:22),decoration:BoxDecoration(border:Border(bottom:BorderSide(color:theme.border))),child:Row(children:[
        CharacterRuntimeView(characterId:target,state:runtimeState,width:60,height:68),
        const SizedBox(width:12),
        Expanded(child:Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Chat with ${member.name}',style:TextStyle(color:theme.text,fontSize:18,fontWeight:FontWeight.w700)),Text(member.role,style:TextStyle(color:theme.mutedText,fontSize:10))])),
        if (showPicker) PopupMenuButton<String>(initialValue:target,onSelected:onSelectAgent,itemBuilder:(_)=>[for(final info in _CharacterChatPageState.members) PopupMenuItem(value:info.id,child:Text('${info.name} · ${info.role}'))]),
        if (busy) SizedBox(width:16,height:16,child:CircularProgressIndicator(strokeWidth:2,color:theme.primary)),
      ])),
...
class _MessageBubble extends StatelessWidget {
  ...
  const _MessageBubble({required this.message,required this.displayName});
  @override
  Widget build(BuildContext context) {
    final theme=CriterivoxTheme.of(context); final user=message.sender=='user';
    return Align(alignment:user?Alignment.centerRight:Alignment.centerLeft,child:Container(constraints:const BoxConstraints(maxWidth:680),margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:user?theme.primary.withValues(alpha:.14):theme.surfaceStrong,borderRadius:BorderRadius.circular(16),border:Border.all(color:user?theme.primary.withValues(alpha:.35):theme.border)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[if(!user)...[Text(displayName,style:TextStyle(color:theme.primary,fontSize:10,fontWeight:FontWeight.w700)),const SizedBox(height:5)],Text(message.text,style:TextStyle(color:theme.text,fontSize:12.5,height:1.45))])));
  }
}
...
class _Composer extends StatelessWidget {
  ...
  const _Composer({required this.input,required this.references,required this.busy,required this.name,required this.onSend,required this.onAttach});
  @override
  Widget build(BuildContext context) {
    final theme=CriterivoxTheme.of(context);
    return Container(padding:const EdgeInsets.fromLTRB(18,10,18,18),decoration:BoxDecoration(border:Border(top:BorderSide(color:theme.border))),child:Column(children:[
      if(references.isNotEmpty) SizedBox(height:42,child:ListView.separated(scrollDirection:Axis.horizontal,itemCount:references.length,itemBuilder:(context,index)=>Chip(avatar:const Icon(Icons.attach_file_rounded,size:14),label:Text(references[index]['name'] as String,maxLines:1,overflow:TextOverflow.ellipsis)),separatorBuilder:(_,__)=>const SizedBox(width:6))),
      Row(children:[IconButton(tooltip:'Attach reference',onPressed:busy?null:onAttach,icon:const Icon(Icons.attach_file_rounded)),Expanded(child:TextField(controller:input,minLines:1,maxLines:5,onSubmitted:(_)=>onSend(),decoration:InputDecoration(hintText:'Message $name…'))),const SizedBox(width:8),IconButton.filled(onPressed:busy?null:onSend,icon:const Icon(Icons.arrow_upward_rounded))]),
    ]));
  }
}

class _HandoffChoices extends StatelessWidget {
  final bool busy;
  final VoidCallback onHandoff,onContinue;
  const _HandoffChoices({required this.busy,required this.onHandoff,required this.onContinue});
  @override
  Widget build(BuildContext context) {
    final theme=CriterivoxTheme.of(context);
    return Container(margin:const EdgeInsets.only(bottom:14),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:theme.surfaceStrong,borderRadius:BorderRadius.circular(16),border:Border.all(color:theme.border)),child:Wrap(spacing:8,runSpacing:8,children:[OutlinedButton.icon(onPressed:busy?null:onHandoff,icon:const Icon(Icons.forward_rounded,size:16),label:const Text('Hand over to Dharen')),FilledButton.icon(onPressed:busy?null:onContinue,icon:const Icon(Icons.person_rounded,size:16),label:const Text('Continue here'))]));
  }
}
...
class _TaskCard extends StatelessWidget {
  ...
  const _TaskCard({required this.state,required this.onOpen});
  @override
  Widget build(BuildContext context) {
    final theme=CriterivoxTheme.of(context);
    return Container(margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(12),decoration:BoxDecoration(color:theme.surfaceStrong,borderRadius:BorderRadius.circular(14),border:Border.all(color:theme.border)),child:Row(children:[Expanded(child:Text('${state.taskId} • ${state.taskState??'ACTIVE'}',style:TextStyle(color:theme.text,fontSize:11))),TextButton(onPressed:onOpen,child:const Text('Open workspace'))]));
  }
}
