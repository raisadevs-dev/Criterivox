...
class _S7State extends State<ReasoningResearchBureauPage> {
  ...
  void _connectWebSocket(String sessionId) {
    ...
    try {
      final channel =
          WebSocketChannel.connect(Uri.parse('ws://127.0.0.1:8017/api/s7/ws'));
      _socket = channel;
      _socketSubscription = channel.stream.listen((raw) {
        ...
        if (message['type'] == 'connected') {
          ...
          channel.sink
              .add(jsonEncode({'type': 'subscribe', 'session_id': sessionId}));
  ...
  Future<void> _start() async {
    ...
    try {
      final response =
          await http.post(Uri.parse('http://127.0.0.1:8017/api/s7/sessions'),
  ...
  Future<void> _challenge(String artifactId) async {
    ...
    try {
      ...
      final response = await http.post(
          Uri.parse('http://127.0.0.1:8017/api/s7/sessions/$id/challenge'),
          headers: {'content-type': 'application/json'},
          body:
  ...
  Widget build(BuildContext context) {
    final artifacts =
        (_session?['artifacts'] as List?)?.cast<Map<String, dynamic>>() ??
            const [];
    final events =
        (_session?['events'] as List?)?.cast<Map<String, dynamic>>() ??
            const [];
    return Scaffold(
      body: Stack(children: [
        Container(
          ...
          Expanded(
            ...
            Expanded(
                child: AnimatedSwitcher(
                    ...
                    child: _roomContent(artifacts, events))),
          ])),
        ])),
        if (_busy)
          const Positioned(
  ...
  Widget _topBar() => Padding(
      ...
      child: Row(children: [
        ...
        const Expanded(
          ...
          Text('REASONING RESEARCH BUREAU',
              style: TextStyle(
                  fontSize: 17,
                  fontWeight: FontWeight.w700,
                  letterSpacing: 1.2)),
          Text('S7 • computational intelligence environment',
  ...
  Widget _statusChip() => Row(children: [
        Container(
            ...
            child: Text(
                _session?['status']?.toString().toUpperCase() ?? 'READY',
                style: const TextStyle(fontSize: 11, letterSpacing: .8))),
        const SizedBox(width: 8),
        Icon(Icons.circle,
            size: 9,
            color: _socketConnected ? Colors.greenAccent : Colors.white24),
        const SizedBox(width: 5),
        Text(_socketConnected ? 'LIVE' : 'HTTP',
            style: const TextStyle(color: Colors.white54, fontSize: 10))
      ]);

  Widget _roomRail() => SizedBox(
      width: 150,
      child: Padding(
  ...
  Widget _roomButton(String id, IconData icon, String label) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    Flexible(
                        child: Text(label,
                            style: TextStyle(
                                color:
                                    _room == id ? Colors.white : Colors.white60,
                                fontSize: 12,
                                fontWeight: FontWeight.w600)))
                  ])))));

  Widget _roomContent(
      List<Map<String, dynamic>> artifacts, List<Map<String, dynamic>> events) {
  ...
  Widget _collaborationRoom(List<Map<String, dynamic>> artifacts,
      ...
      Padding(
          ...
          child:
            ...
            _hero(
                'COLLABORATION ROOM',
                'Observe • inspect • compare • challenge • intervene',
                const Color(0xffc6b4ff)),
                  ...
                  child: _glass(
                      ...
                      Column(children: [
                        ...
                        TextField(
                            ...
                            maxLines: 2,
                            decoration:
                                const InputDecoration(labelText: 'Context')),
  ...
  Widget _inspectionRoom(List<Map<String, dynamic>> artifacts) => Padding(
      padding: const EdgeInsets.fromLTRB(8, 8, 24, 24),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _hero(
            'VIVREN ROOM',
            'Critical inspection • assumptions • defects • epistemic boundaries',
        ...
        Expanded(
            child: _glass(
                ...
                artifacts.isEmpty
                    ? const _EmptyState()
                    : _artifactList(artifacts,
                        filter: {'evaluation', 'limitation'})))
      ]));

  Widget _explorationRoom(List<Map<String, dynamic>> artifacts) => Padding(
      padding: const EdgeInsets.fromLTRB(8, 8, 24, 24),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _hero('TARKIS ROOM', 'Explore • branch • compare • reflect • refine',
            const Color(0xffffbd72)),
        Expanded(
            child: _glass(
                'HYPOTHESIS / BRANCH EXPLORATION',
                artifacts.isEmpty
                    ? const _EmptyState()
                    : _artifactList(artifacts,
                        filter: {'hypothesis', 'reasoning', 'result'})))
      ]));

  Widget _hero(String title, String subtitle, Color accent) => Padding(
  ...
  Widget _glass(String title, Widget child) => Container(
  ...
  Widget _artifactList(List<Map<String, dynamic>> artifacts,
      {Set<String>? filter}) {
    final shown = filter == null
        ? artifacts
        : artifacts.where((a) => filter.contains(a['kind'])).toList();
    return ListView.separated(
        ...
        itemBuilder: (_, i) {
          ...
          return ListTile(
              ...
              title: Text('${a['title']}  ·  v${a['version']}'),
              subtitle: Text(
                  '${a['kind']}  •  branch ${a['branch_id']}\n${a['content']}'),
              trailing: IconButton(
                  tooltip: _socketConnected
                      ? 'Challenge over live channel'
                      : 'Challenge',
                  onPressed: () => _challenge(a['artifact_id']),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Center(
      child: Text('No analytical activity yet. Submit a task to begin.',
...
class _ChallengeDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AlertDialog(
          title: const Text('Challenge analytical artifact'),
          content: TextField(
              controller: _controller,
              maxLines: 4,
              decoration: const InputDecoration(
                  hintText: 'State the substantive challenge...')),
          actions: [
            TextButton(
