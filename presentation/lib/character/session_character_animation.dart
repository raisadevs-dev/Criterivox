...
class SessionCharacterMotion {
  final double duration, phase, sway, lift, emphasis;
  final int direction;
...
class SessionCharacterAnimationView extends StatefulWidget {
  final String characterId, state;
  ...
  const SessionCharacterAnimationView(
      ...
      this.height = 286});
  @override
  State<SessionCharacterAnimationView> createState() =>
      _SessionCharacterAnimationViewState();
}

class _SessionCharacterAnimationViewState
    extends State<SessionCharacterAnimationView>
  ...
  void didUpdateWidget(covariant SessionCharacterAnimationView oldWidget) {
    ...
    if (oldWidget.characterId != widget.characterId) {
      ...
      _controller.duration =
          Duration(milliseconds: (_motion.duration * 1000).round());
    }
    _updateTicker(oldWidget.reducedMotion);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final visualState = CharacterAnimationStateMapper.bloomSignal(
        CharacterAnimationStateMapper.fromRuntime(
            characterState: widget.state));
    return AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          final active = visualState != 'IDLE';
          final lift = wave * _motion.lift * (active ? 1 : .65);
  ...
  String generatedSvgFrame({required int frame, int frameCount = 8}) {
    ...
    final visualState = CharacterAnimationStateMapper.bloomSignal(
        CharacterAnimationStateMapper.fromRuntime(
            characterState: widget.state));
    return GeneratedVectorAnimation(
            profile: profile,
            sessionSeed: SessionCharacterAnimation.sessionSeed)
        .svgFrame(state: visualState, frame: frame, frameCount: frameCount);
  }
}
