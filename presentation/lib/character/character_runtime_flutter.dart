...
final double gesture =
    state == 'COMMUNICATE' ? math.sin(t * 2.2) : 0.0;
...
final double handY =
    63.0 + (state == 'COMMUNICATE' ? gesture * 3.0 : 0.0);
...
final double sway =
    math.sin(time) * 1.8 + attention * 0.4;
...
final bool blink =
    math.sin(progress * math.pi * 4).abs() > 0.985;
...
if (state == 'COMMUNICATE') {
  canvas.drawOval(
    ...
    Paint()
      ..color = character.dark.withValues(alpha: 0.7),
  );
}
}
