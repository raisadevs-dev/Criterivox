...
class _RoomPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    for (var i = 0; i < 7; i++) c.drawLine(Offset(i * s.width / 6, 62), Offset(i * s.width / 6, horizon), panel);
    ...
    for (var i = -6; i <= 6; i++) c.drawLine(Offset(s.width / 2 + i * 55, horizon), Offset(s.width / 2 + i * 175, s.height), floor);
    for (var i = 1; i < 7; i++) {
...
class _MonitorPainter extends CustomPainter { final S8Room room; _MonitorPainter(this.room); @override void paint(Canvas c,Size s){final a=_accent(room);c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(3,2,46,28),const Radius.circular(4)),Paint()..color=const Color(0xFF0D1720));final p=Paint()..color=a.withValues(alpha:.65)..style=PaintingStyle.stroke..strokeWidth=1.2;c.drawRect(Rect.fromLTWH(7,6,38,20),p);c.drawLine(const Offset(10,20),const Offset(18,14),p);c.drawLine(const Offset(18,14),const Offset(27,18),p);c.drawLine(const Offset(27,18),const Offset(40,10),p);c.drawLine(const Offset(10,23),const Offset(42,23),p);c.drawLine(const Offset(26,30),const Offset(26,35),Paint()..color=const Color(0xFF71808A)..strokeWidth=2);c.drawLine(const Offset(18,36),const Offset(34,36),Paint()..color=const Color(0xFF71808A)..strokeWidth=2);} @override bool shouldRepaint(covariant _MonitorPainter old)=>old.room!=room; }
...
class _PaperPainter extends CustomPainter { final S8Room room; _PaperPainter(this.room); @override void paint(Canvas c,Size s){for(var i=0;i<3;i++){final r=Rect.fromLTWH(3+i*2,4+i*2,22,25);c.drawRect(r,Paint()..color=const Color(0xFFD7D0C4));}final ink=Paint()..color=_accent(room).withValues(alpha:.55)..strokeWidth=1;for(var i=0;i<4;i++)c.drawLine(Offset(7,10+i*4),Offset(22,10+i*4),ink);} @override bool shouldRepaint(covariant _PaperPainter old)=>old.room!=room; }
...
class _DrinkPainter extends CustomPainter { final S8Room room; _DrinkPainter(this.room); @override void paint(Canvas c,Size s){final a=_accent(room);c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(4,7,17,17),const Radius.circular(4)),Paint()..color=const Color(0xFFB8A486));c.drawArc(Rect.fromLTWH(18,10,9,11),-math.pi/2,math.pi,false,Paint()..color=const Color(0xFFB8A486)..style=PaintingStyle.stroke..strokeWidth=2);c.drawOval(Rect.fromLTWH(5,5,15,5),Paint()..color=const Color(0xFF473528));c.drawCircle(const Offset(12,15),2,Paint()..color=a.withValues(alpha:.45));} @override bool shouldRepaint(covariant _DrinkPainter old)=>old.room!=room; }
...
class _PlantPainter extends CustomPainter { final PlantKind kind; _PlantPainter(this.kind); @override void paint(Canvas c,Size s){final stem=Paint()..color=const Color(0xFF4B8A63)..strokeWidth=3..strokeCap=StrokeCap.round;final leaf=Paint()..color=const Color(0xFF2D7654);final light=Paint()..color=const Color(0xFF5C9C6E);c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(9,35,24,14),const Radius.circular(4)),Paint()..color=const Color(0xFF705039));c.drawLine(const Offset(21,36),const Offset(21,16),stem);switch(kind){case PlantKind.cactus:c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(15,10,12,27),const Radius.circular(8)),leaf);c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(8,20,9,6),const Radius.circular(4)),light);c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(25,18,9,6),const Radius.circular(4)),light);break;case PlantKind.palm:for(var i=0;i<7;i++)_leaf(c,const Offset(21,16),i*math.pi/3.3,light,14);break;case PlantKind.flower:for(var i=0;i<6;i++)c.drawCircle(Offset(21+math.cos(i*math.pi/3)*7,15+math.sin(i*math.pi/3)*7),5,light);c.drawCircle(const Offset(21,15),3,Paint()..color=const Color(0xFFD7B66A));for(var i=0;i<5;i++)_leaf(c,Offset(21,22+i*2),i*.9,leaf,9);break;default:for(var i=0;i<9;i++)_leaf(c,Offset(21,16+(i%3)*3),i.isEven?-.8:.8, i.isEven?leaf:light, 8+(i%3)*2);}}
...
class _TallPlantPainter extends CustomPainter { final PlantKind kind; _TallPlantPainter(this.kind); @override void paint(Canvas c,Size s){final p=Paint()..color=const Color(0xFF2C7955);final pot=Paint()..color=const Color(0xFF6B4B35);c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(17,76,36,23),const Radius.circular(6)),pot);c.drawLine(const Offset(35,79),const Offset(35,32),Paint()..color=const Color(0xFF4A895F)..strokeWidth=4);if(kind==PlantKind.palm){for(var i=0;i<9;i++){c.save();c.translate(35,32);c.rotate(-1.25+i*.31);c.drawOval(Rect.fromLTWH(0,-4,34,8),p);c.restore();}}else if(kind==PlantKind.cactus){c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(27,20,17,59),const Radius.circular(9)),p);c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(17,43,14,9),const Radius.circular(6)),p);c.drawRRect(RRect.fromRectAndRadius(const Rect.fromLTWH(42,35,14,9),const Radius.circular(6)),p);}else{for(var i=0;i<13;i++){final y=30+(i%5)*8;final x=35+(i.isEven?-(i%3)*6:(i%3)*6);c.drawOval(Rect.fromLTWH(x,y,24,9),p);}}} @override bool shouldRepaint(covariant _TallPlantPainter old)=>old.kind!=kind; }
...
class _PanelPainter extends CustomPainter { final S8Room room; _PanelPainter(this.room); @override void paint(Canvas c,Size s){final a=_accent(room);final p=Paint()..color=a.withValues(alpha:.38)..style=PaintingStyle.stroke..strokeWidth=1.4;for(var i=0;i<4;i++)c.drawLine(Offset(8,15+i*12),Offset(s.width-8,15+i*12),p);c.drawCircle(const Offset(20,19),5,Paint()..color=a.withValues(alpha:.75));c.drawRect(const Rect.fromLTWH(62,10,38,27),p);c.drawLine(const Offset(68,47),Offset(s.width-10,47),p);c.drawCircle(const Offset(88,57),6,p);} @override bool shouldRepaint(covariant _PanelPainter old)=>old.room!=room; }
...
class _DecorPainter extends CustomPainter { final DecorKind kind; _DecorPainter(this.kind); @override void paint(Canvas c,Size s){final p=Paint()..style=PaintingStyle.stroke..strokeWidth=2..color=const Color(0xFFB8C9D3);final fill=Paint()..color=const Color(0xFF203342);final a=const Color(0xFF69C8EA);final cx=s.width/2,cy=s.height/2;switch(kind){case DecorKind.flask:c.drawPath(Path()..moveTo(cx-5,5)..lineTo(cx+5,5)..lineTo(cx+5,13)..lineTo(cx+12,27)..quadraticBezierTo(cx,34,cx-12,27)..lineTo(cx-5,13)..close(),p);c.drawCircle(Offset(cx,25),3,Paint()..color=a);break;case DecorKind.book:c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(6,8,s.width-12,s.height-14),const Radius.circular(3)),fill);c.drawLine(Offset(cx,9),Offset(cx,s.height-7),p);c.drawLine(Offset(10,14),Offset(cx-3,14),p);c.drawLine(Offset(cx+3,14),Offset(s.width-10,14),p);break;case DecorKind.archive:c.drawRect(Rect.fromLTWH(6,7,s.width-12,s.height-13),fill);for(var i=0;i<3;i++)c.drawLine(Offset(10,14+i*7),Offset(s.width-10,14+i*7),p);break;case DecorKind.calculator:c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(6,5,s.width-12,s.height-10),const Radius.circular(5)),fill);c.drawRect(Rect.fromLTWH(10,9,s.width-20,8),p);for(var i=0;i<6;i++)c.drawCircle(11+(i%3)*8,22+(i~/3)*7,1.8,Paint()..color=a);break;case DecorKind.scale:c.drawLine(Offset(cx,7),Offset(cx,29),p);c.drawLine(Offset(7,12),Offset(s.width-7,12),p);c.drawLine(const Offset(10,13),Offset(5,27),p);c.drawLine(Offset(s.width-10,13),Offset(s.width-5,27),p);c.drawArc(Rect.fromLTWH(0,22,10,5),0,math.pi,p);c.drawArc(Rect.fromLTWH(s.width-10,22,10,5),0,math.pi,p);break;case DecorKind.search:c.drawCircle(Offset(cx-3,cy-3),9,p);c.drawLine(Offset(cx+4,cy+4),Offset(cx+12,cy+12),p);break;case DecorKind.link:c.drawOval(Rect.fromLTWH(3,10,17,10),p);c.drawOval(Rect.fromLTWH(14,10,17,10),p);break;case DecorKind.calendar:c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(5,6,s.width-10,s.height-10),const Radius.circular(4)),p);c.drawLine(Offset(5,14),Offset(s.width-5,14),p);for(var i=0;i<6;i++)c.drawCircle(11+(i%3)*8,20+(i~/3)*8,1.5,Paint()..color=a);break;case DecorKind.orb:c.drawCircle(Offset(cx,cy),12,Paint()..color=a.withValues(alpha:.10));c.drawCircle(Offset(cx,cy),12,p);c.drawOval(Rect.fromCenter(center:Offset(cx,cy),width:25,height:10),p);c.drawLine(Offset(cx,cy-12),Offset(cx,cy+12),p);break;case DecorKind.controller:c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(5,10,s.width-10,17),const Radius.circular(9)),fill);c.drawCircle(Offset(12,18),2,Paint()..color=a);c.drawCircle(Offset(s.width-12,17),2,Paint()..color=a);c.drawLine(Offset(cx-2,18),Offset(cx+4,18),p);c.drawLine(Offset(cx+1,15),Offset(cx+1,21),p);}}
