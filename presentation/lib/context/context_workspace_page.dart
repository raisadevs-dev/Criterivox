...
class ContextWorkspacePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final graph = state?.provenanceGraph;
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('The context layer is inspectable: provenance, context change, evidence completeness, memory validity and agent activity are explicit runtime state.', style: TextStyle(color: theme.mutedText, fontSize: 12, height: 1.5)),
          ...
          Wrap(spacing: 12, runSpacing: 12, children: [
            ...
            _Metric(label: 'Evidence completeness', value: state?.evidenceCompleteness == null ? 'UNKNOWN' : '${state!.evidenceCompleteness}%', theme: theme),
          ]),
          ...
          _Section(title: 'CONTEXT PROVENANCE GRAPH', theme: theme, child: _GraphView(graph: graph, theme: theme)),
          const SizedBox(height: 14),
          _Section(title: 'CONTEXT DIMENSIONS', theme: theme, child: Wrap(spacing: 8, runSpacing: 8, children: [for (final item in const ['content','creator','platform','temporal','audience','environment']) _Pill(label: item, active: state?.contextDimensions.contains(item) ?? false, theme: theme)])),
          const SizedBox(height: 14),
          _Section(title: 'CONTEXT DIFF', theme: theme, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _DiffRow(label: 'Added', values: _strings(diff?['added']), theme: theme),
            _DiffRow(label: 'Removed', values: _strings(diff?['removed']), theme: theme),
            _DiffRow(label: 'Changed', values: _strings(diff?['changed']), theme: theme),
            _DiffRow(label: 'Unchanged', values: _strings(diff?['unchanged']), theme: theme),
            const SizedBox(height: 8),
            Text('Semantic equivalence is deliberately not claimed by a structural diff.', style: TextStyle(color: theme.mutedText, fontSize: 9.5)),
          ])),
          const SizedBox(height: 14),
          _Section(title: 'EVIDENCE DEBT', theme: theme, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Text(state?.evidenceCompleteness == null ? 'UNKNOWN' : '${state!.evidenceCompleteness}%', style: TextStyle(color: theme.text, fontSize: 24, fontWeight: FontWeight.w800)), const SizedBox(width: 12), _Pill(label: debtLevel, active: debtLevel == 'LOW', theme: theme)]),
            const SizedBox(height: 10),
            Wrap(spacing: 7, runSpacing: 7, children: [for (final tag in state?.evidenceTags ?? const <String>[]) _Pill(label: tag, active: false, theme: theme)]),
            const SizedBox(height: 8),
            Text('HEURISTIC • the percentage is an evidence-completeness indicator, not a confidence probability. Threshold effectiveness remains unvalidated.', style: TextStyle(color: theme.mutedText, fontSize: 9.5, height: 1.4)),
          ])),
          const SizedBox(height: 14),
          _Section(title: 'CONTEXT MEMORY WITH EXPIRATION', theme: theme, child: _KeyValueGrid(theme: theme, values: {
            'Status': memoryStatus,
            'Recheck at': state?.memoryRecheckAt ?? 'UNKNOWN',
            'Reason': state?.memoryRecheckReason ?? 'No research-validated recheck rule supplied',
            'Rule': 'No universal TTL is assumed',
          })),
          const SizedBox(height: 14),
          _Section(title: 'AGENT OBSERVABILITY TIMELINE', theme: theme, child: state?.observabilityEvents.isNotEmpty == true
              ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [for (final event in state!.observabilityEvents.take(20)) _ActivityEvent(event: event, theme: theme)])
              : Text('No structured activity events have been emitted yet.', style: TextStyle(color: theme.mutedText, fontSize: 10.5))),
          const SizedBox(height: 14),
          _Section(title: 'CONTROLLED MULTI-AGENT HANDOFF', theme: theme, child: _KeyValueGrid(theme: theme, values: {
          ...
          _Section(title: 'INTERPRETATION', theme: theme, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if ((state?.contextUncertainty ?? const <String>[]).isNotEmpty) ...[const SizedBox(height: 8), Text('UNCERTAINTY  ${state!.contextUncertainty.join(' • ')}', style: TextStyle(color: theme.mutedText, fontSize: 10, height: 1.5))],
          ...
          const SizedBox(height: 14),
          _Section(title: 'MCP-READY CAPABILITY BOUNDARY', theme: theme, child: Text('IMPLEMENTED: Criterivox exposes a protocol-neutral internal capability boundary. MCP is kept as a future adapter concern, with no autonomous external tool execution implied.', style: TextStyle(color: theme.mutedText, fontSize: 11, height: 1.5))),
        ]),
      ),
    );
  }
...
class _GraphView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final nodes = graph?['nodes'] is List ? (graph!['nodes'] as List).whereType<Map>().map((item) => Map<String,dynamic>.from(item)).toList() : const <Map<String,dynamic>>[];
    if (nodes.isEmpty) return Text('No provenance graph has been built yet.', style: TextStyle(color: theme.mutedText, fontSize: 10.5));
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Wrap(spacing: 8, runSpacing: 8, children: [for (final node in nodes) _GraphNode(label: node['label']?.toString() ?? 'Unknown', kind: node['kind']?.toString() ?? '', theme: theme)]),
      const SizedBox(height: 10),
      Text('${nodes.length} provenance node(s) • traceability is IMPLEMENTED • immutability is UNKNOWN', style: TextStyle(color: theme.mutedText, fontSize: 9.5)),
    ]);
  }
}
...
class _GraphNode extends StatelessWidget {
  ...
  final String kind;
  final CriterivoxTheme theme;
  const _GraphNode({required this.label, required this.kind, required this.theme});
  @override
...
class _DiffRow extends StatelessWidget {
  ...
  final CriterivoxTheme theme;
  const _DiffRow({required this.label, required this.values, required this.theme});
  @override
