...
class S8FourRoomBureau extends StatefulWidget {
  ...
  const S8FourRoomBureau({super.key, this.initialRoom = S8Room.home, this.roomContent, this.motionEnabled = true});
...
class _S8FourRoomBureauState extends State<S8FourRoomBureau> {
  ...
  @override
  void initState() { super.initState(); _room = widget.initialRoom; }
  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        Expanded(child: AnimatedSwitcher(
          ...
          child: S8AnimatedRoomEnvironment(
            key: ValueKey(_room),
            room: _room,
...
class _RoomRail extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(height: 82, child: ListView.separated(
    ...
    itemBuilder: (context, index) {
      ...
      return Semantics(button: true, selected: selectedRoom, label: '${meta.name}: ${meta.role}', child: InkWell(
        onTap: () => onSelected(room), borderRadius: BorderRadius.circular(18),
        child: AnimatedContainer(duration: 220.ms, width: 188, padding: const EdgeInsets.all(12), decoration: BoxDecoration(
          ...
          const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(meta.name, style: const TextStyle(fontWeight: FontWeight.w800, letterSpacing: .7)), const SizedBox(height: 2),
...
class _DefaultRoomContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(meta.icon, color: meta.accent), const SizedBox(width: 10), Text(meta.name.toUpperCase(), style: TextStyle(color: meta.accent, fontWeight: FontWeight.w900, letterSpacing: 2))]),
      const SizedBox(height: 8), Text(meta.description, style: const TextStyle(fontSize: 14, color: Colors.white70)),
      const Spacer(), Wrap(spacing: 8, runSpacing: 8, children: [for (final capability in meta.capabilities) Chip(avatar: Icon(meta.icon, size: 14, color: meta.accent), label: Text(capability))]),
    ]);
  }
}

class _RoomMeta {
  final String name; final String role; final String description; final List<String> capabilities; final IconData icon; final Color accent;
