...
import 'home01_feature_console.dart';
...
class _ShellState extends State<CriterivoxShell> {
  ...
  @override void initState() {
    ...
    _stateSubscription = runtime.states.listen((value) {
      if (!mounted) return;
      setState(() { state = value; busy = false; if (value.taskId != null) { _history.removeWhere((item) => item.taskId == value.taskId && item.agentId == value.agentId); _history.insert(0, value); } });
    });
    _errorSubscription = runtime.errors.listen((error) { if (!mounted) return; setState(() => busy = false); ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content: Text(error))); });
    if (widget.connectRuntime) runtime.connect();
  }
  @override void dispose() { _stateSubscription.cancel(); _errorSubscription.cancel(); if (widget.runtimeClient == null) runtime.dispose(); task.dispose(); data.dispose(); ctx.dispose(); super.dispose(); }
  void open(String value) => setState(() => page = value);
  void openDoorAddress(String? address) { if (address == null || address.trim().isEmpty) { open('workspace'); return; } final uri = Uri.tryParse(address); if (uri == null) { open('workspace'); return; } final path = uri.path; setState(() { routeHighlight = uri.queryParameters['highlight']; page = path == '/workspace/sandre' || path == '/workspace/sandre/' ? 'stewardship' : 'workspace'; }); if (routeHighlight != null && routeHighlight!.isNotEmpty) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sandre focus: $routeHighlight'))); }
  ...
  void handleBloomCapability(BloomCapability capability) { if (capability == BloomCapability.stewardship) return; if (capability != BloomCapability.analyze) showReserved(Bloom.labels[capability]!); }
  void handoffFromBloom() { final foundationId = state?.foundationId; final confirmed = state?.foundationConfirmation == 'user-confirmed' || state?.foundationConfirmation == 'user-corrected'; if (foundationId == null || !confirmed) { open('stewardship'); return; } setState(() => busy = true); runtime.dataAction(foundationId: foundationId, action: 'handoff', recipient: 'dharen'); open('workspace'); }
  void send(String message,{String? target,List<Map<String,dynamic>> references = const []}) { final selected = target ?? chatTarget; setState(() { busy = true; chatTarget = selected; }); runtime.sendChat(message: message,targetCharacter: selected,taskId: state?.taskId,data: {'dataset': data.text,'records': 3},context: {'description': ctx.text,'origin': selected == 'dharen' ? 'Direct Dharen Chat' : selected == 'sandre' ? 'Direct Sandre Chat' : 'Syvax Routing'},references: references); }
  void start() { final taskText = task.text.trim(); if (taskText.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter an analysis task before starting.'))); return; } setState(() => busy = true); runtime.requestApplication(intent: 'analyze',task: taskText,taskId: state?.taskId,data: {'dataset': data.text,'records': 3},context: {'description': ctx.text,'origin': 'Analysis Workspace'},source: 'workspace'); }
