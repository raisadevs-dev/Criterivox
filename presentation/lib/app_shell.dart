...
class _ShellState extends State<CriterivoxShell> {
  ...
  @override void initState() {super.initState();_stateSubscription=runtime.states.listen((value){if(!mounted)return;setState((){state=value;busy=false;});});_errorSubscription=runtime.errors.listen((error){if(!mounted)return;setState(()=>busy=false);ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content:Text(error)));});if(widget.connectRuntime)runtime.connect();}
  ...
  void handleBloomCapability(BloomCapability capability){if(capability==BloomCapability.stewardship){open('stewardship');return;}if(capability!=BloomCapability.analyze)showReserved(Bloom.labels[capability]!);}
  void handoffFromBloom(){final id=state?.foundationId;final confirmed=state?.foundationConfirmation=='user-confirmed'||state?.foundationConfirmation=='user-corrected';if(id==null||!confirmed){open('stewardship');return;}setState(()=>busy=true);runtime.dataAction(foundationId:id,action:'handoff',recipient:'dharen');open('workspace');}
  void send(String message,{String? target,List<Map<String,dynamic>> references=const []}){final selected=target??chatTarget;setState((){busy=true;chatTarget=selected;});runtime.sendChat(message:message,targetCharacter:selected,taskId:state?.taskId,data:{'dataset':data.text,'records':3},context:{'description':ctx.text,'origin':selected=='dharen'?'Direct Dharen Chat':'Syvax Routing'},references:references);}
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor:t.page,body:SafeArea(child:Row(children:[
      ...
      Expanded(child:Column(children:[
        _TopBar(isDarkMode:widget.isDarkMode,onToggleTheme:widget.onToggleTheme,connectionLive:state!=null,onSearch:(value){final q=value.trim().toLowerCase();if(q.isNotEmpty&&((state?.taskId??'').toLowerCase().contains(q)||(state?.task??'').toLowerCase().contains(q)))open('workspace');}),
        Expanded(child:AnimatedSwitcher(duration:const Duration(milliseconds:260),child:
          ...
          page=='chat'?CharacterChatPage(key:const ValueKey('chat'),state:state,busy:busy,selectedAgent:chatTarget,onSelectAgent:(agent)=>setState(()=>chatTarget=agent),onSend:(message,agent,references)=>send(message,target:agent,references:references),onOpenTask:()=>open('workspace')):
          page=='workspace'?AnalysisPage(key:const ValueKey('workspace'),state:state,busy:busy,task:task,data:data,contextText:ctx,onStart:start,onChat:()=>open('chat')):
          page=='stewardship'?DataStewardshipPage(key:const ValueKey('stewardship'),state:state,runtime:runtime):
          BloomPage(key:const ValueKey('bloom'),state:state,onCapability:handleBloomCapability,onSub:(value)=>open(value==BloomSuboption.workspace?'workspace':'chat'),onSyvax:(message)=>send(message,target:'syvax'),onStewardship:()=>open('stewardship'),onHandoff:handoffFromBloom,busy:busy),
        )),
      ])),
    ])));
  }
...
class _Sidebar extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){final t=CriterivoxTheme.of(context);final width=expanded?244.0:76.0;return AnimatedContainer(duration:const Duration(milliseconds:240),width:width,decoration:BoxDecoration(color:t.surface.withValues(alpha:.96),border:Border(right:BorderSide(color:t.border))),child:Column(children:[
    ...
    Expanded(child:Scrollbar(thumbVisibility:expanded,child:SingleChildScrollView(padding:EdgeInsets.symmetric(horizontal:expanded?12:8),child:Column(children:[
      _StatusCard(expanded:expanded),const SizedBox(height:18),_section('START HERE',expanded,t),_nav('App Introduction',Icons.auto_awesome_rounded,page=='intro',()=>onOpen('intro'),expanded,t),const SizedBox(height:8),_section('NAVIGATION',expanded,t),
      _nav('Bloom',Icons.spa_rounded,page=='bloom',()=>onOpen('bloom'),expanded,t),_nav('Data Stewardship',Icons.inventory_2_rounded,page=='stewardship',()=>onOpen('stewardship'),expanded,t),_nav('Analysis Workspace',Icons.dashboard_customize_rounded,page=='workspace',()=>onOpen('workspace'),expanded,t),_nav('Character Chat',Icons.forum_rounded,page=='chat',()=>onOpen('chat'),expanded,t),const SizedBox(height:16),_section('FUTURE CAPABILITIES',expanded,t),
      _nav('Compare',Icons.balance_rounded,false,()=>onReserved('Compare'),expanded,t),_nav('Explore',Icons.search_rounded,false,()=>onReserved('Explore'),expanded,t),_nav('Plan',Icons.calendar_month_rounded,false,()=>onReserved('Plan'),expanded,t),_nav('Insights',Icons.lightbulb_outline_rounded,false,()=>onReserved('Insights'),expanded,t),_nav('Explain',Icons.chat_bubble_outline_rounded,false,()=>onReserved('Explain'),expanded,t),
      if(expanded)Padding(padding:const EdgeInsets.all(10),child:Text('Navigation moves between surfaces. Bloom discovers capabilities; Data Stewardship is Sandre\'s working surface.',style:TextStyle(color:t.mutedText,fontSize:10,height:1.45))),
    ]))),
  ]));}
