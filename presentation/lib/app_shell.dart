...
class _ShellState extends State<CriterivoxShell> {
  ...
  void showReserved(String capability) {
    ScaffoldMessenger.of(context)
      ..hideCurrentSnackBar()
      ..showSnackBar(SnackBar(content: Text('$capability is reserved for a future capability sprint.')));
  }
  ...
  void start() {
    ...
    if (taskText.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter an analysis task before starting.')),
      );
      return;
    }
    setState(() => busy = true);
                ...
                children: [
                  _TopBar(
                    ...
                    onSearch: (value) {
                      ...
                      if (q.isNotEmpty &&
                          ((state?.taskId ?? '').toLowerCase().contains(q) ||
                              (state?.task ?? '').toLowerCase().contains(q))) {
                        open('workspace');
                      }
                    },
                  ),
                      ...
                      child: page == 'intro'
                          ...
                          : page == 'chat'
                              ? CharacterChatPage(
                                  ...
                                  onSend: (message, agent, references) => send(
                                    message,
                                    target: agent,
                                    references: references,
                                  ),
                              ...
                              : page == 'workspace'
                                  ...
                                  : BloomPage(
                                      ...
                                      onSub: (value) => open(
                                        value == BloomSuboption.workspace ? 'workspace' : 'chat',
                                      ),
                                      onSyvax: (message) => send(message, target: 'syvax'),
                                      busy: busy,
                                    ),
                    ),
                  ),
...
class _Sidebar extends StatelessWidget {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    _nav('App Introduction', Icons.auto_awesome_rounded, page == 'intro', () => onOpen('intro'), expanded, t),
                    const SizedBox(height: 8),
                    _section('NAVIGATION', expanded, t),
                    _nav('Bloom', Icons.spa_rounded, page == 'bloom', () => onOpen('bloom'), expanded, t),
                    ...
                    if (expanded)
                      Padding(
                        ...
                        child: Text(
                          'Navigation moves between surfaces. Bloom provides capability discovery; it does not duplicate this traffic-control layer.',
  ...
  Widget _section(String text, bool visible, CriterivoxTheme t) => visible
      ? Align(
          ...
          child: Padding(
            ...
            child: Text(
              text,
              style: TextStyle(color: t.mutedText, fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 1.1),
            ),
          ),
        )
      : const SizedBox(height: 8);
      ...
      child: Row(
        children: [
            ...
            child: SizedBox(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  prefixIcon: Icon(Icons.search_rounded, color: t.mutedText, size: 19),
                  hintText: 'Search current task by name or ID...',
                ),
              ),
            ),
          ),
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.circle, size: 7, color: connectionLive ? t.success : t.warning),
          ...
          IconButton(
            ...
            icon: Icon(isDarkMode ? Icons.light_mode_rounded : Icons.dark_mode_rounded, color: t.mutedText),
          ),
        ],
      ),
    );
              ...
              Transform.rotate(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: const LinearGradient(colors: [Color(0xFF9A7BFF), Color(0xFF6654E8)]),
                  ),
                ),
              ),
            Container(
              width: size * .24,
              height: size * .24,
              decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xFF7D68F7)),
            ),
          ],
        ),
      );
