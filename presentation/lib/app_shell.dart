...
class _ShellState extends State<CriterivoxShell> {
    ...
    _stateSubscription = runtime.states.listen((value) {
      ...
      setState(() {
        ...
        if (value.taskId != null) {
          _history.removeWhere((item) => item.taskId == value.taskId && item.agentId == value.agentId);
  ...
  @override void dispose() {
    _stateSubscription.cancel(); _errorSubscription.cancel();
    if (widget.runtimeClient == null) runtime.dispose();
    task.dispose(); data.dispose(); ctx.dispose(); super.dispose();
  }

  void open(String value) => setState(() => page = value);

  void openDoorAddress(String? address) {
    if (address == null || address.trim().isEmpty) { open('workspace'); return; }
    final uri = Uri.tryParse(address);
    if (uri == null) { open('workspace'); return; }
    final path = uri.path;
    setState(() {
  ...
  void handleBloomCapability(BloomCapability capability) {
    ...
    if (capability != BloomCapability.analyze) showReserved(Bloom.labels[capability]!);
  }

  void handoffFromBloom() {
    final foundationId = state?.foundationId;
    final confirmed = state?.foundationConfirmation == 'user-confirmed' || state?.foundationConfirmation == 'user-corrected';
    if (foundationId == null || !confirmed) { open('stewardship'); return; }
    setState(() => busy = true);
    runtime.dataAction(foundationId: foundationId, action: 'handoff', recipient: 'dharen');
    open('workspace');
  }
  ...
  void send(String message,{String? target,List<Map<String,dynamic>> references = const []}) {
    ...
    runtime.sendChat(message: message,targetCharacter: selected,taskId: state?.taskId,data: {'dataset': data.text,'records': 3},context: {'description': ctx.text,'origin': selected == 'dharen' ? 'Direct Dharen Chat' : 'Syvax Routing'},references: references);
  }

  void start() {
    final taskText = task.text.trim();
    if (taskText.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter an analysis task before starting.'))); return; }
    setState(() => busy = true);
    runtime.requestApplication(intent: 'analyze',task: taskText,taskId: state?.taskId,data: {'dataset': data.text,'records': 3},context: {'description': ctx.text,'origin': 'Analysis Workspace'},source: 'workspace');
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: t.page,body: SafeArea(child: Row(children:[
      ...
      Expanded(child: Column(children:[
        _TopBar(isDarkMode: widget.isDarkMode,onToggleTheme: widget.onToggleTheme,connectionLive: state != null,onSearch: (value){final q=value.trim().toLowerCase();final found=_history.where((item)=>(item.taskId??'').toLowerCase().contains(q)||(item.task??'').toLowerCase().contains(q)).toList();if(q.isNotEmpty&&found.isNotEmpty){setState(()=>state=found.first);open('workspace');}}),
        Expanded(child: AnimatedSwitcher(duration: const Duration(milliseconds:260),child:
          ...
          page == 'dharen' ? DharenHomePage(key: const ValueKey('dharen'),history:_history,onOpenTask:(item)=>openDoorAddress(item.doorAddress ?? '/workspace?task_id=${item.taskId}&focus=analysis')) :
