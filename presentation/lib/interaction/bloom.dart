...
class Bloom extends StatefulWidget {
  ...
  static const subtitles = <BloomCapability, String>{
    ...
    BloomCapability.stewardship: 'Protect the data foundation',
    BloomCapability.compare: 'Compare across contexts',
    BloomCapability.explore: 'Discover patterns',
    BloomCapability.plan: 'Deliberate on next steps',
    BloomCapability.insights: 'Surface useful insight',
...
class _BloomState extends State<Bloom> with SingleTickerProviderStateMixin {
  ...
  Widget _center(
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Container(
          width: compact ? size * 0.34 : size * 0.27,
          height: compact ? size * 0.34 : size * 0.27,
      ...
      child: Semantics(
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            width: compact ? 94 : 138,
            height: compact ? 94 : 138,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              gradient: RadialGradient(
                colors: [
      ...
      child: Tooltip(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: t.surfaceStrong.withValues(
                alpha: 0.96,
              ),
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
...
class _BloomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      final accent = accentMap[capability]!;
...
The specific analyzer warning from the original was:

```dart
final chatAction = capability == BloomCapability.stewardship
    ? BloomSuboption.stewardshipChat
    : BloomSuboption.chat;
```

It was **never used**. I removed it rather than inventing behavior around it.

Everything else in the supplied Bloom interaction is retained, including:

* capability selection
* owner mapping
* Workspace action
* owner-chat action
* stewardship-specific Workspace routing
* animated Bloom center
* radial capability nodes
...
One architectural detail is worth noting: the original `BloomSuboption.stewardshipChat` and `BloomSuboption.chat` values still exist because they are part of the public enum/API, even though this widget currently routes the owner button through `onOwnerChat`. I did **not** silently delete those API values just to make the analyzer happy.
*/
