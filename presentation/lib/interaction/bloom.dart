...
class Bloom extends StatefulWidget {
  ...
  static const subtitles = <BloomCapability, String>{
    ...
    BloomCapability.stewardship: 'Sandre safeguards the foundation',
  ...
  static const icons = <BloomCapability, IconData>{
    ...
    BloomCapability.stewardship: Icons.inventory_2_rounded,
  ...
  static const accents = <BloomCapability, Color>{
    ...
    BloomCapability.stewardship: Color(0xFF40E7D0),
...
class _BloomState extends State<Bloom> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context){final t=CriterivoxTheme.of(context);return LayoutBuilder(builder:(context,constraints){final availableWidth=math.min(constraints.maxWidth,820.0);final hasBoundedHeight=constraints.hasBoundedHeight;final compact=availableWidth<600;final ratio=compact?.98:.82;final heightLimitedSize=hasBoundedHeight?constraints.maxHeight/ratio:double.infinity;final size=math.max(240.0,math.min(availableWidth,heightLimitedSize));final height=size*ratio;return SizedBox(width:size,height:height,child:Stack(clipBehavior:Clip.none,alignment:Alignment.center,children:[AnimatedBuilder(animation:_pulse,builder:(context,child)=>CustomPaint(size:Size(size,height),painter:_BloomPainter(selected:widget.selected,pulse:_pulse.value,accentMap:Bloom.accents))),_center(size,compact,t),..._nodes(size,compact),if(expanded==BloomCapability.analyze)_suboptions(size,compact)]));});}
  Widget _center(double size,bool compact,CriterivoxTheme t)=>AnimatedBuilder(animation:_pulse,builder:(context,child)=>Container(width:compact?size*.34:size*.27,height:compact?size*.34:size*.27,decoration:BoxDecoration(shape:BoxShape.circle,gradient:RadialGradient(colors:[t.primary.withValues(alpha:.55),t.surfaceStrong,t.page]),border:Border.all(color:t.primary,width:2),boxShadow:[BoxShadow(color:t.primary.withValues(alpha:.26),blurRadius:34+_pulse.value*14,spreadRadius:5+_pulse.value*3)]),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[const _BloomMark(size:38),const SizedBox(height:8),Text('Criterivox',style:TextStyle(fontSize:21,fontWeight:FontWeight.w600,color:t.text)),const SizedBox(height:5),Text(expanded==null?'What would you like\nto do today?':'${Bloom.labels[expanded!]}\nchoose a path',textAlign:TextAlign.center,style:TextStyle(color:t.text,fontSize:11.5,height:1.35))])));
  List<Widget> _nodes(double size,bool compact){final radius=compact?size*.34:size*.32;final cy=compact?size*.49:size*.44;final d=compact?94.0:138.0;final count=BloomCapability.values.length;return[for(var i=0;i<count;i++)Positioned(left:size/2+math.cos(-math.pi/2+i*2*math.pi/count)*radius-d/2,top:cy+math.sin(-math.pi/2+i*2*math.pi/count)*radius-d/2,child:_Node(capability:BloomCapability.values[i],compact:compact,selected:expanded==BloomCapability.values[i],onTap:()=>_select(BloomCapability.values[i])))];}
  Widget _suboptions(double size,bool compact){final center=Offset(size/2,compact?size*.49:size*.44);final radius=compact?size*.21:size*.22;return Stack(children:[Positioned(left:center.dx-radius-70,top:center.dy+(compact?2:10),child:_Suboption(icon:Icons.dashboard_customize_rounded,title:'Workspace',subtitle:'Deep analysis',accent:Bloom.accents[BloomCapability.analyze]!,onTap:()=>widget.onSuboption?.call(BloomSuboption.workspace))),Positioned(left:center.dx+radius-70,top:center.dy+(compact?2:10),child:_Suboption(icon:Icons.forum_rounded,title:'Chat',subtitle:'Choose a character',accent:Bloom.accents[BloomCapability.explain]!,onTap:()=>widget.onSuboption?.call(BloomSuboption.chat))) ]);}
}

class _Node extends StatelessWidget{final BloomCapability capability;final bool compact;final bool selected;final VoidCallback onTap;const _Node({required this.capability,required this.compact,required this.selected,required this.onTap});@override Widget build(BuildContext context){final t=CriterivoxTheme.of(context);final accent=Bloom.accents[capability]!;final d=compact?94.0:138.0;final reserved=capability!=BloomCapability.analyze&&capability!=BloomCapability.stewardship;return Material(color:Colors.transparent,child:Semantics(button:true,label:'${Bloom.labels[capability]} capability${reserved?', reserved':''}',child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(d),child:AnimatedContainer(duration:const Duration(milliseconds:240),width:d,height:d,decoration:BoxDecoration(shape:BoxShape.circle,gradient:RadialGradient(colors:[accent.withValues(alpha:selected?.35:.12),t.surfaceStrong]),border:Border.all(color:accent.withValues(alpha:selected?1:.58),width:selected?2.4:1.2),boxShadow:[BoxShadow(color:accent.withValues(alpha:selected?.35:.10),blurRadius:selected?30:18,spreadRadius:selected?4:1)]),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[Icon(Bloom.icons[capability],color:accent,size:compact?23:28),const SizedBox(height:6),Text(Bloom.labels[capability]!,textAlign:TextAlign.center,style:TextStyle(color:t.text,fontSize:compact?12:15,fontWeight:FontWeight.w700)),if(!compact)...[const SizedBox(height:4),SizedBox(width:d-34,child:Text(Bloom.subtitles[capability]!,textAlign:TextAlign.center,style:TextStyle(color:t.mutedText,fontSize:9.5,height:1.25)))] ])))));}}

class _Suboption extends StatelessWidget{final IconData icon;final String title;final String subtitle;final Color accent;final VoidCallback onTap;const _Suboption({required this.icon,required this.title,required this.subtitle,required this.accent,required this.onTap});@override Widget build(BuildContext context){final t=CriterivoxTheme.of(context);return Material(color:Colors.transparent,child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(17),child:Container(width:140,height:68,padding:const EdgeInsets.symmetric(horizontal:10),decoration:BoxDecoration(color:t.surfaceStrong.withValues(alpha:.97),borderRadius:BorderRadius.circular(17),border:Border.all(color:accent.withValues(alpha:.72)),boxShadow:[BoxShadow(color:accent.withValues(alpha:.16),blurRadius:22)]),child:Row(children:[Container(width:36,height:36,decoration:BoxDecoration(shape:BoxShape.circle,color:accent.withValues(alpha:.13)),child:Icon(icon,color:accent,size:19)),const SizedBox(width:8),Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:TextStyle(color:t.text,fontSize:12.5,fontWeight:FontWeight.w700)),const SizedBox(height:3),Text(subtitle,style:TextStyle(color:t.mutedText,fontSize:8.8))])]))) ;}}
