...
async def syvax_steer(payload:dict):
 task_id=str(payload.get('task_id','')); correction=str(payload.get('correction','')); return {**syvax_engine.steer(task_id,correction),'runtime':home03_services.suspend(task_id,correction)}
...
async def conversation_message(conversation_id:str,payload:dict):
...
async def conversation_tree(conversation_id:str): return home03_store.tree(conversation_id)
@router.post('/api/home03/conversation/{conversation_id}/branch')
async def conversation_branch(conversation_id:str,payload:dict):
 bid=str(payload.get('branch_id') or f'branch-{int(__import__("time").time()*1000)}'); return {'branch_id':home03_store.branch(bid,conversation_id,int(payload.get('parent_message',0)),str(payload.get('name','New branch')),dict(payload.get('state',{})))}
@router.post('/api/home03/checkpoint')
async def home03_checkpoint(payload:dict):
 state=dict(payload.get('state',{})); import hashlib,json; h=hashlib.sha256(json.dumps(state,sort_keys=True,default=str).encode()).hexdigest(); cid=home03_store.checkpoint(str(payload.get('conversation_id','default')),str(payload.get('branch_id','main')),state,h); return {'checkpoint_id':cid,'state_hash':h}
@router.post('/api/home03/replay')
async def home03_replay(payload:dict): return {'replay_supported':True,'mode':'checkpoint-branch','checkpoint_id':payload.get('checkpoint_id'),'branch_id':payload.get('branch_id')}
