...
class RuntimeConnectionManager:
    ...
    async def connect(self, websocket: Any) -> None:
        await websocket.accept(); self.clients.add(websocket); await websocket.send_text(json.dumps(self.latest.to_dict()))
...
def _task_source(source: str) -> AnalysisTaskSource:
    try: return AnalysisTaskSource(source.lower())
...
async def handle_chat_message(payload: Any) -> None:
    ...
    if task_id is None:
        ...
        raw_refs=payload.get("references",[]); references=tuple(x for x in raw_refs if isinstance(x,str)) if isinstance(raw_refs,list) else ()
        ...
        await dharen_runtime.publish_task(task,message="Dharen received your analysis request from chat.",event="CHAT_ANALYSIS_REQUESTED"); asyncio.create_task(analysis_tasks.execute(task.task_id)); return
