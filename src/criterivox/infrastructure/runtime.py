...
from criterivox.character_backbone.capability_discovery import discover
...
class AnalysisRequest(BaseModel):
    ...
    def validate_payload_size(self):
        if len(self.data) > 1000 or len(self.context) > 1000:
            raise ValueError(
                "Runtime payload contains too many top-level fields."
            )

        if len(json.dumps(self.model_dump(), default=str)) > 32000:
...
class RuntimeConnectionManager:
    ...
    async def connect(self, websocket):
        ...
        await websocket.send_text(
            json.dumps(self._with_foundation(self.latest).to_dict())
        )
    ...
    def _with_foundation(self, contract):
        if not self.latest_foundation:
            return contract

        values = {
            key: value
            for key, value in self.latest_foundation.items()
            if getattr(contract, key, None) in (None, (), [])
        }
    ...
    async def publish(self, contract):
        ...
        if foundation_fields:
            self.latest_foundation = foundation_fields

        contract = self._with_foundation(contract)
...
class DharenRuntime:
    ...
    def __init__(self, connection_manager):
        ...
        self._activity = CharacterActivityManager(
            CHARACTER_REGISTRY.get_all()
        )
        self._legacy_lock = asyncio.Lock()

    async def publish_task(self, task, *, message=None, event=None):
        cs = self._CHARACTER_STATES[task.state]

        active = (
            cs is not CharacterState.IDLE
            or task.state is AnalysisTaskState.COMPLETED
        )

        activity = self._activity.set_state("Dharen", cs)

        result = task.result

        refs = (
            tuple(r.name for r in task.reference_details)
            or tuple(task.references)
        )

        await self._connections.publish(
    ...
    async def _publish_idle(self, task):
        activity = self._activity.set_state(
            "Dharen",
            CharacterState.IDLE,
        )
        ...
        await self._connections.publish(
            PresentationContract.from_state(
                ...
                prominence=0.25,
                message=(
                    "Dharen is idle. The completed analysis remains "
    ...
    def _task_message(self, task):
        return {
            ...
            AnalysisTaskState.VALIDATING: (
                "Validating the supplied data, context, and references."
            ),
            AnalysisTaskState.PROCESSING: (
                "Preparing the supplied information for analysis."
            ),
            AnalysisTaskState.ANALYZING: (
                "Dharen is analyzing the supplied information and "
                "establishing contextual findings."
            ),
            AnalysisTaskState.RESULT_READY: (
                "The analysis result is ready for communication."
            ),
            AnalysisTaskState.COMPLETED: "Dharen completed the analysis.",
            AnalysisTaskState.WAITING: (
                "The analysis is waiting for required information."
            ),
            AnalysisTaskState.FAILED: (
                task.error or "The analysis failed."
            ),
    ...
    async def run_analysis(self, request, *, result=None):
        async with self._legacy_lock:
            ...
            result = result or self._perform_synthetic_analysis(request)
            ...
            await self._transition(
                ...
                message=(
                    f"Analysis completed: {result['data_items']} "
                    f"data items across {result['data_fields']} fields; "
    ...
    @staticmethod
    def _perform_synthetic_analysis(request):
        return {
            "data_items": len(request.data),
            "data_fields": sum(
                len(x) if isinstance(x, dict) else 1
                for x in request.data.values()
            ),
    ...
    for i, item in enumerate(raw):
        if isinstance(item, str):
            ...
            try:
                ...
                raise ValueError(
                    "Attached reference content is not valid base64."
                ) from exc
            ...
            if (
                ...
                raise ValueError(
                    "Attached reference size does not match its payload."
                )
            ...
            if total > MAX_REFERENCE_BATCH_BYTES:
                raise ValueError(
                    "Attached references exceed the 8 MB chat batch limit."
                )
...
def _foundation_payload(refs, details, message, task_id):
    ...
    for name, detail in zip(refs, details):
        ...
        if decoded is not None:
            ...
            item["content_base64"] = detail.content_base64

        sources.append(item)

    return {
        "sources": sources,
        "collection_id": task_id or f"chat-{id(message)}",
        "supplied_context": {
            "entered_through": "Syvax Chatbox",
            "chat_message": message[:500],
            "task_id": task_id,
            "material_origin": "chat_reference",
        },
    }


async def _sync_chat_material(
    refs,
    details,
    message,
    task_id,
    ...
    fields = {
        ...
        "foundation_confirmation": (
            foundation.confirmation_status.value
        ),
        "foundation_preview_question": (
            "Is this what you intended to submit?"
        ),
        "foundation_recipient": "syvax",
    }
    ...
    await runtime_connections.publish(
        PresentationContract.from_state(
            ...
            prominence=0.9,
            message=(
                f"Sandre received {len(foundation.sources)} "
                "chat material source(s). Extraction is now "
                "available in Data Stewardship."
            ),
    ...
    await runtime_connections.publish(
        PresentationContract.from_state(
            ...
            prominence=0.9,
            message=(
                "Sandre completed the initial chat-material "
                "extraction and profiling. The same foundation "
                "is now visible to the stewardship workspace."
            ),
...
async def handle_application_request(payload):
    ...
    if request.intent.value != "analyze":
        raise UnsupportedCapabilityError(
            f"Capability '{request.intent.value}' "
    ...
    if request.task_id:
        ...
        await dharen_runtime.publish_task(
            task,
            message=(
                "The existing analysis task was reopened "
                "from another surface."
            ),
            event="ANALYSIS_TASK_REOPENED",
        )

        if not task.is_terminal:
            asyncio.create_task(
                analysis_tasks.execute(task.task_id)
            )
    ...
    await dharen_runtime.publish_task(
        task,
        message=(
            "Analysis task created. Dharen is ready to receive it."
        ),
        event="ANALYSIS_TASK_CREATED",
    )
...
async def handle_chat_message(payload):
    ...
    if target not in ALLOWED_CHAT_CHARACTERS:
        raise ValueError("Unknown chat character.")
    ...
    refs, details = _parse_chat_references(
        payload.get("references", [])
    )

    await _sync_chat_material(
        refs,
        details,
        message,
        task_id,
    ...
    # Characters other than Syvax and Dharen are now handled
    # through the character-backbone capability registry rather
    # than the deleted legacy application.character_chat module.
    if target not in {"syvax", "dharen"}:
        ...
        if not matches:
            await _publish_character(
                ...
                message=(
                    f"No registered capability is currently "
                    f"available for {target} for the requested "
                    "operation."
                ),
        ...
        if not implemented:
            await _publish_character(
                ...
                CharacterState.WARNING,
                message=(
                    f"{target} is registered for this capability, "
                    "but the capability is not currently executable "
                    "in this runtime."
                ),
        ...
        await _publish_character(
            ...
            CharacterState.COMMUNICATE,
            message=(
                f"{target} identified the registered capability "
                f"{implemented[0].capability_id}."
            ),
    ...
    if target == "syvax":
        if interpretation.intent in {
            ...
            if task_id is None:
                await _publish_character(
                    ...
                    message=(
                        ...
                        "so there is no authoritative state record "
                        "to inspect."
                    ),
                    event="STATE_QUERY_NO_TASK",
                )
                return
            ...
            try:
                message_out, structured = respond_state_query(
                    tid,
                    interpretation,
                )
            except Exception:
                message_out = (
                    "No authoritative runtime record exists "
                    "for that state."
                )
                structured = {
                    "status": "NO_AUTHORITATIVE_RECORD"
                }
            ...
            await _publish_character(
                ...
                CharacterState.COMMUNICATE,
                message=message_out,
                event="STATE_AWARE_RESPONSE",
                task=(
                    analysis_tasks.get_task(tid)
                    if tid in analysis_tasks.store.tasks
                    else None
                ),
            )
        ...
        if interpretation.intent in {
            ...
            if task_id is None:
                await _publish_character(
                    ...
                    message=(
                        "No task is bound to this conversation, "
                        "so no workflow interruption was performed."
                    ),
                    event="INTERRUPTION_NO_TASK",
                )
                return

            tid = str(task_id)
            task = analysis_tasks.get_task(tid)
            ...
            if interpretation.intent == "pause":
                if task.is_terminal:
                    await _publish_character(
                        ...
                        event="PAUSE_UNSUPPORTED",
                        task=task,
                    )
                    return

                state_runtime.pause(tid)

                await _publish_character(
                    "Syvax",
                    CharacterState.COMMUNICATE,
                    message=(
                        "The task has been paused through "
                        "the runtime gate."
                    ),
                    event="TASK_PAUSED",
                    task=task,
            ...
            if interpretation.intent == "resume":
                state_runtime.resume(tid)

                await _publish_character(
                    "Syvax",
                    CharacterState.COMMUNICATE,
                    message=(
                        "The task has been resumed through "
                        "the runtime gate."
                    ),
                    event="TASK_RESUMED",
                    task=task,
            ...
            if interpretation.intent == "cancel":
                await _publish_character(
                    ...
                    CharacterState.WARNING,
                    message=(
                        "Cancellation is not supported by the "
                        "current safe character-chat boundary."
                    ),
                    event="CANCEL_UNSUPPORTED",
                    task=task,
                )
            ...
            await _publish_character(
                ...
                CharacterState.COMMUNICATE,
                message=(
                    "The requested change was recorded. "
                    "Downstream state was not silently modified."
                ),
        ...
        if interpretation.intent == "handoff":
            if task_id is None:
                task = analysis_tasks.create_task(
                    task=interpretation.normalized_text,
                    data={},
                    context={},
                    source=AnalysisTaskSource.CHAT,
                    references=refs,
            ...
            await _publish_character(
                "Syvax",
                CharacterState.RECEIVE,
                message="Syvax received your handoff instruction.",
                event="SYVAX_RECEIVED",
                task=task,
            )
            ...
            await _publish_character(
                ...
                CharacterState.WORK,
                message=(
                    "Syvax is preparing the handoff context "
                    "for Dharen."
                ),
            ...
            await _publish_character(
                ...
                message=(
                    "Syvax can hand this task to Dharen. "
                    "The current task, data, context, and "
                    "references will remain attached."
                ),
            ...
            await _publish_character(
                "Dharen",
                CharacterState.RECEIVE,
                message="Dharen received the task from Syvax.",
        ...
        if interpretation.intent == "user_continue":
            if task_id is None:
                ...
                await _publish_character(
                    ...
                    CharacterState.COMMUNICATE,
                    message=(
                        "Syvax will keep the current task with "
                        "you. No handoff was performed."
                    ),
                    event="USER_CONTINUES",
                    task=task,
                )
        ...
        if interpretation.intent == "analyze":
            ...
            await _publish_character(
                "Syvax",
                CharacterState.RECEIVE,
                message="Syvax received your analysis request.",
                event="SYVAX_RECEIVED",
                task=task,
            )
            ...
            await _publish_character(
                ...
                CharacterState.COMMUNICATE,
                message=(
                    "This is an analysis task. Syvax recommends "
                    "Dharen for the structural analysis. You can "
                    "hand it over to Dharen or continue yourself."
                ),
                event="HANDOFF_PROPOSED",
                task=task,
            )
        ...
        await _publish_character(
            ...
            CharacterState.RECEIVE,
            message=(
                "Syvax received your request and is interpreting "
                "what you need."
            ),
            event="SYVAX_RECEIVED",
            task=task,
        ...
        await _publish_character(
            ...
            CharacterState.COMMUNICATE,
            message=(
                "I can route analysis work to Dharen, or you "
                "can continue the task yourself. Tell me which "
                "you prefer."
            ),
            event="HANDOFF_PROPOSED",
            task=task,
    ...
    task = (
        analysis_tasks.get_task(str(task_id))
    ...
    if task is None:
        ...
        await dharen_runtime.publish_task(
            task,
            message="Dharen received your request directly.",
            event="CHAT_ANALYSIS_REQUESTED",
        )
    ...
    if details:
        ...
        task.add_activity(
            f"Attached {len(details)} additional reference(s) "
            "to the task."
        )

    task.add_activity(
        f"User asked Dharen: {interpretation.normalized_text}"
    )
    ...
    if interpretation.intent == "status":
        await dharen_runtime.publish_task(
            task,
            message=(
                "Dharen reports the authoritative analysis state: "
                f"{task.state.value}."
            ),
        ...
        await dharen_runtime.publish_task(
            task,
            message=(
                "Dharen confirms the current analysis remains "
                "active. The same task state and evidence are preserved."
            ),
            event="TASK_CONTINUE_REQUESTED",
        )
        ...
        await dharen_runtime.publish_task(
            task,
            message=(
                "Dharen received the follow-up. The task remains "
                "grounded in its recorded data, context, and references."
            ),
...
async def _publish_character(
    ...
    *,
    ...
    activity = activity_manager.set_state(
        character_id,
        state,
    )
