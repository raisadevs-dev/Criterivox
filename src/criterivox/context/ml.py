...
except ImportError:  # optional ML runtime
    joblib = None
...
class LearnedContextModel:
    ...
    def load(self) -> bool:
        if joblib is None or self.model_path is None or not self.model_path.exists():
            return False
...
class AnukaLearnedModel(LearnedContextModel):
    """Learned adaptation-trigger classifier. Safety and state gates remain deterministic."""
...
def train_text_model(texts: list[str], labels: list[str], output_path: str | Path) -> ModelMetrics:
    if Pipeline is None or joblib is None:
        raise RuntimeError("Install the optional ML dependencies before training.")
...
__all__ = ["DharenLearnedModel", "AnukaLearnedModel", "LearnedContextModel", "ModelMetrics", "train_text_model", "save_training_report"]
