...
class AnalysisTaskService:
 ...
 async def _move(self,task,state,activity):
  previous=task.state.value;task.transition(state);task.add_activity(activity);state_runtime.record_event(task.task_id,'STATE_CHANGED',actor='dharen',previous_state=previous,new_state=state.value,provenance={'activity':activity});ordered=('CREATED','RECEIVED','VALIDATING','PROCESSING','ANALYZING','RESULT_READY','COMPLETED');idx=ordered.index(state.value) if state.value in ordered else -1;completed_steps=ordered[:idx+1] if idx>=0 else (state.value,);remaining_steps=ordered[idx+1:] if idx>=0 else ();state_runtime.checkpoint(task.task_id,current_step=state.value,active_step=state.value,completed_steps=completed_steps,remaining_steps=remaining_steps,active_character='dharen',active_capability='analysis',state=state.value,event_refs=tuple(e.event_id for e in state_runtime.events(task.task_id)[-5:]));await self._publish(task)
