...
async def handle_character_chat(payload: dict) -> None:
    ...
    machine = unified.machine\n    unified_fields = {\n        "unified_journey_id": journey_id, "unified_intent": unified.language.intent,\n        "unified_entities": unified.language.entities, "unified_target": unified.language.target,\n        "unified_requested_output": unified.language.requested_output, "unified_confidence": unified.language.confidence,\n        "unified_source": unified.language.source, "unified_capability": machine.get("capability"),\n        "unified_responsible_character": machine.get("responsible_character"), "unified_authorization": machine.get("authorization"),\n        "unified_state_source": machine.get("state_source"), "unified_workflow_outcome": machine.get("workflow_outcome"),\n        "unified_status": machine.get("status"), "unified_detected_language": unified.language.detected_language, "unified_response_language": unified.language.response_language, "unified_language_confidence": unified.language.language_confidence,\n    }\n    await runtime_connections.publish(PresentationContract.from_state(target, CharacterState.COMMUNICATE, active=True, prominence=.9, message=response, event="CHARACTER_CHAT_RESPONSE", task_id=task_id, activity=activity(), observability_events=traces(), failure_id=failure_event.event_id if failure_event else None, failure_type=failure_event.failure_type.value if failure_event else None, **unified_fields))
    ...
    state_runtime.record_event(task_id, "CHARACTER_CHAT_RESPONSE", actor=target, capability=machine.get("capability"), status=str(machine.get("status") or "RECORDED"), provenance={"journey_id": journey_id, "intent": unified.language.intent})\n    state_runtime.checkpoint(task_id, current_step="character_chat", active_step=None, completed_steps=("character_chat",), remaining_steps=(), active_character=target, active_capability=machine.get("capability"), state="COMPLETED")\n    OBSERVABILITY.record(task_id=task_id, character_id=target, action="IDLE", reason="Interaction completed.")
