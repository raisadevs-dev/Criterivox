...
async def _safe_context_build(payload: dict) -> None:
    try:
        ...
        common["observability_events"] = tuple(item.to_dict() for item in events); common["activity"] = tuple(f"{item.character_id}: {item.action} • {item.reason}" for item in events)
...
async def _safe_foundation_sync(payload: dict) -> None:
    try:
        ...
        await runtime_connections.publish(PresentationContract.from_state("sandre", CharacterState.WARNING, active=True, prominence=.88, message=f"Browser foundation recovery was rejected: {exc}", event="FOUNDATION_SYNC_REJECTED", foundation_id=payload.get("foundation_id"), foundation_revision=payload.get("revision"), foundation_authoritative=False))
        raise

# S5 browser-recovery compatibility alias. The browser's foundation_sync envelope
# is the authoritative recovery protocol; S6 consumes the restored foundation via
# the normal DataFoundation -> AnalysisTask -> DharenRuntime path above.
...
async def _safe_data_action(payload: dict) -> None:
    try:
        ...
        await runtime_connections.publish(PresentationContract.from_state("sandre", CharacterState.WARNING, active=True, prominence=.9, message=f"Data action could not be completed: {exc}", event="DATA_ACTION_FAILED"))
...
async def _publish_foundation_state(character: str, state: CharacterState, message: str, event: str, foundation, *, preview=None, recipient=None, conflict_fields=(), log_count=None, log_entries=(), conditional_provenance=(), feature_payload=None) -> None:
    ...
    if feature_payload is not None: kwargs["s5_feature_payload"] = feature_payload
...
async def _safe_character_chat(payload: dict) -> None:
    try:
        await handle_character_chat(payload)
    except Exception as exc:
        logger.exception("Character chat failed.")
        await runtime_connections.publish(PresentationContract.from_state("sandre", CharacterState.WARNING, active=True, prominence=.8, message=f"Character chat could not be completed: {exc}", event="CHARACTER_CHAT_FAILED"))

async def _safe_runtime_payload(payload: dict) -> None:
    try:
        request = parse_analysis_request(payload)
        await dharen_runtime.run_analysis(request)
    except Exception as exc:
    ...
    try:
        while True:
            ...
            if isinstance(payload, dict) and payload.get("type") == "foundation_sync":
                ...
                await _safe_request(handle_chat_message, payload)
            ...
            else: asyncio.create_task(_safe_runtime_payload(payload))
