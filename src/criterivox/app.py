...
async def _safe_context_build(payload: dict) -> None:
    try:
        ...
        _context_snapshots[foundation_id] = {item.key: item.value for item in context.items}
        events = []

        def trace(action: str, reason: str, *, output: str | None = None) -> None:
            events.append(observability.record(task_id=task_id, character_id="dharen", action=action, reason=reason, context_id=context.context_id, output=output))

        trace("RECEIVE", "Received validated S5 foundation for contextual structuring.")
        common = {
            "foundation_id": foundation_id, "foundation_material_set_id": foundation_id, "task_id": task_id, "context_id": context.context_id,
            "context_dimensions": dimensions, "context_missing_dimensions": missing, "context_normalization_count": len(result.normalization),
            "context_baseline_id": baseline.baseline_id, "context_baseline_status": baseline.status.value, "provenance_graph": _graph_payload(result.provenance_graph),
            "context_diff": _diff_payload(result.context_diff), "evidence_completeness": result.evidence_debt.completeness_percent, "evidence_debt_level": result.evidence_debt.level.value,
            "evidence_tags": result.evidence_debt.tags, "memory_status": result.memory.status, "memory_recheck_at": result.memory.expires_at.isoformat() if result.memory.expires_at else None,
            "memory_recheck_reason": memory_reason, "observability_events": tuple(item.to_dict() for item in events), "activity": tuple(f"{item.character_id}: {item.action} • {item.reason}" for item in events),
        ...
        await runtime_connections.publish(PresentationContract.from_state("dharen", CharacterState.RECEIVE, active=True, prominence=.9, message=f"Dharen received foundation {foundation_id} for contextual structuring.", event="CONTEXT_BUILD_RECEIVED", **common))
        await asyncio.sleep(.1)
        trace("WORK", "Structured dimensions while preserving S5 lineage.")
        common["observability_events"] = tuple(item.to_dict() for item in events)
        common["activity"] = tuple(f"{item.character_id}: {item.action} • {item.reason}" for item in events)
        await runtime_connections.publish(PresentationContract.from_state("dharen", CharacterState.WORK, active=True, prominence=.9, message="Dharen is structuring contextual dimensions and preserving the S5 lineage reference.", event="CONTEXT_BUILD_WORKING", **common))
        await asyncio.sleep(.1)
        trace("COMMUNICATE", "Context build completed and is ready for inspection.", output=result.interpretation.interpretation)
        common.update({"context_interpretation_id": result.interpretation.interpretation_id, "context_uncertainty": result.interpretation.uncertainty, "context_limitations": result.interpretation.limitations, "observability_events": tuple(item.to_dict() for item in events), "activity": tuple(f"{item.character_id}: {item.action} • {item.reason}" for item in events)})
        await runtime_connections.publish(PresentationContract.from_state("dharen", CharacterState.COMMUNICATE, active=True, prominence=.9, message=result.interpretation.interpretation, event="CONTEXT_BUILD_COMPLETE", **common))
    except Exception as exc:
...
async def _safe_data_action(payload: dict) -> None:
    try:
        ...
        if action in {"confirm", "correct", "exclude", "add", "irrelevant", "clarify"}:
            ...
            await _publish_foundation_state("sandre", CharacterState.COMMUNICATE, f"Recorded user review as {foundation.confirmation_status.value}. The source remains preserved.", f"USER_INFORMATION_{action.upper()}", foundation, preview=stewardship.preview(foundation)); return
        ...
        if action == "log_search":
            entries = stewardship.search_logs(str(payload.get("query", ""))); latest = data_foundations.get(foundation_id) if foundation_id else None
            if latest:
                formatted = tuple(f"{entry.timestamp} • {entry.event} • {entry.material_set_id} • tasks={','.join(entry.task_ids) or 'none'} • {entry.detail}" for entry in entries[:20]); await _publish_foundation_state("sandre", CharacterState.COMMUNICATE, f"Found {len(entries)} stewardship log record(s).", "STEWARD_LOG_SEARCH", latest, log_count=len(entries), log_entries=formatted)
            return
        if action == "merge_conflicts":
            ...
            merged, resolutions = stewardship.merge_conflicts(home, chat, {str(k): str(v) for k, v in winners.items()}); foundation = data_foundations.get(foundation_id); fields = tuple(item.field for item in resolutions); stewardship.record(foundation, event="CONFLICTS_RESOLVED", detail=f"Resolved {len(resolutions)} field conflict(s) explicitly."); await _publish_foundation_state("sandre", CharacterState.COMPLETE, f"Merged {len(resolutions)} conflicting field(s) using explicit user choices. No hard overwrite was applied.", "CONFLICTS_RESOLVED", foundation, conflict_fields=fields, log_entries=tuple(f"{k}: {v!r}" for k, v in merged.items())); return
    ...
    try:
        while True:
            ...
            if isinstance(payload, dict) and payload.get("type") == "chat_message":
                ...
                if target in PROFILES and target not in {"dharen", "syvax"}: asyncio.create_task(_safe_request(handle_character_chat, payload))
                else: asyncio.create_task(_safe_request(handle_chat_message, payload))
            elif isinstance(payload, dict) and payload.get("type") == "context_build": asyncio.create_task(_safe_context_build(payload))
            elif isinstance(payload, dict) and payload.get("type") == "scratchpad_signoff":
                task_id = str(payload.get("task_id", "")).strip()
                removed = sign_off_task_scratchpad(task_id)
                await runtime_connections.publish(PresentationContract.from_state("kaelen", CharacterState.COMPLETE, active=True, prominence=.8, message=f"Scratchpad sign-off completed for {task_id or 'UNBOUND'}; {len(removed)} temporary item(s) cleared.", event="SCRATCHPAD_SIGNED_OFF", task_id=task_id or None))
                ...
                request = parse_analysis_request(payload); asyncio.create_task(dharen_runtime.run_analysis(request))
    except WebSocketDisconnect:
        runtime_connections.disconnect(websocket)
    except (ValueError, TypeError):
        await websocket.close(code=1003, reason="Invalid runtime payload"); runtime_connections.disconnect(websocket)
    except Exception:
        logger.exception("Character runtime connection failed."); runtime_connections.disconnect(websocket)
